...
pub use validation::{
    canonicalize_capability_support_row, canonicalize_contract_set,
    canonicalize_domain_coverage, canonicalize_support_claim_set,
...
pub enum ConformanceContractError {
    ...
    #[error(
        "claim observation for {domain:?} is {claimed:?}, but domain coverage is {observed:?}"
    )]
