...
use crate::{
    ...
    ImplementationSupport, MAX_SET_ITEMS, MAX_SUPPORT_ROWS, MAX_TEXT_BYTES,
...
pub fn canonicalize_domain_coverage(
    ...
    for row in &mut coverage {
        ...
        canonicalize_text_set(
            "domain_coverage.blind_boundaries",
            &mut row.blind_boundaries,
        )?;
        canonicalize_text_set(
            "domain_coverage.invalidation_set",
            &mut row.invalidation_set,
        )?;
    }
...
/// evidence satisfies the row; use [`validate_capability_support_row_against_coverage`]
/// for that.
...
/// Validates the intrinsic shape and combinations of one support row.
...
pub fn canonicalize_support_claim_set(
    ...
    validate_support_claim_set_against_coverage(&rows, coverage)?;
    Ok(rows)
}

/// Validates an already canonical support claim set's intrinsic identities and shapes.
pub fn validate_support_claim_set(
    rows: &[CapabilitySupportRow],
) -> Result<(), ConformanceContractError> {
    validate_support_claim_set_with(rows, validate_capability_support_row)
}

/// Validates an already canonical support claim set against exact coverage.
fn validate_support_claim_set_against_coverage(
    rows: &[CapabilitySupportRow],
    coverage: &[DomainCoverage],
) -> Result<(), ConformanceContractError> {
    validate_domain_coverage(coverage)?;
    validate_support_claim_set_with(rows, |row| {
        validate_capability_support_row_against_coverage(row, coverage)
    })
}

fn validate_support_claim_set_with(
    rows: &[CapabilitySupportRow],
    mut validate_row: impl FnMut(&CapabilitySupportRow) -> Result<(), ConformanceContractError>,
) -> Result<(), ConformanceContractError> {
    validate_collection_bound("support_rows", rows.len(), MAX_SUPPORT_ROWS)?;
...
pub fn canonicalize_contract_set(
    ...
    contract_set.domain_coverage = canonicalize_domain_coverage(contract_set.domain_coverage)?;
    contract_set.support_rows =
        canonicalize_support_claim_set(contract_set.support_rows, &contract_set.domain_coverage)?;
...
pub fn validate_conformance_contract_set(
    ...
    validate_support_claim_set_against_coverage(
        &contract_set.support_rows,
...
fn validate_capability_support_row_shape(
    ...
    validate_optional_text(
        "support.proof_profile_ref",
        row.proof_profile_ref.as_deref(),
    )?;
    validate_optional_text(
    ...
    let claim_domain =
        row.claim_domain
            .ok_or(ConformanceContractError::InvalidSupportCombination {
...
fn validate_domain_time(row: &DomainCoverage) -> Result<(), ConformanceContractError> {
    if let Some(expires_at_ms) = row.expires_at_ms {
        let observed_at_ms = row
            .observed_at_ms
            .ok_or(ConformanceContractError::InvalidTime {
...
fn support_row_key(row: &CapabilitySupportRow) -> (&str, &str, Option<EvidenceDomain>, &str) {
    (
