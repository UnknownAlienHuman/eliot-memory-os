...
use eliot_conformance_contracts::{
    CONTRACT_VERSION, CapabilitySupportRow, ConformanceContractError, ConformanceContractSet,
    ContractMaturity, DomainCoverage, EvidenceDomain, EvidenceExecutionStatus,
    ImplementationSupport, SupportObservationState, canonicalize_capability_support_row,
    ...
    validate_conformance_contract_set, validate_domain_coverage, validate_support_claim_set,
};
use serde_json::json;
...
fn current_verified_requires_executed_unblinded_exact_evidence() {
    ...
    validate_capability_support_row(&row).expect("intrinsic row shape is valid");
    row.evidence_execution_status = EvidenceExecutionStatus::NotExecuted;
    assert!(matches!(
        validate_capability_support_row_against_coverage(&row, &domains),
...
fn not_applicable_has_no_domain_dependency_or_execution_claim() {
    ...
    validate_capability_support_row_against_coverage(&row, &domains)
        .expect("valid not-applicable row");
...
fn canonicalization_is_permutation_invariant_and_strict_validation_rejects_drift() {
    ...
    left.support_rows[0].source_handles = vec!["source:z".to_owned(), "source:a".to_owned()];

    let mut right = contract_set();
    right.support_rows[0].source_handles = vec!["source:a".to_owned(), "source:z".to_owned()];
    ...
    assert!(matches!(
        validate_capability_support_row_against_coverage(&noncanonical, &coverage()),
        Err(ConformanceContractError::NonCanonicalCollection { .. })
    ));
...
fn claim_identity_is_unique_but_distinct_domains_may_share_contract_and_scope() {
    ...
    validate_support_claim_set(&rows).expect("canonical rows validate");
...
#[test]
fn intrinsic_set_passes_but_coverage_bound_rejects_invalid_and_unverified_coverage() {
    let rows = vec![verified_row()];
    validate_support_claim_set(&rows)
        .expect("intrinsically valid rows must pass intrinsic set validation");
    validate_capability_support_row(&rows[0])
        .expect("intrinsically valid row must pass intrinsic row validation");

    let mut stale_coverage = coverage();
    let runtime = stale_coverage
        .iter_mut()
        .find(|row| row.domain == EvidenceDomain::Runtime)
        .unwrap();
    runtime.state = SupportObservationState::Stale;

    let err = canonicalize_support_claim_set(rows.clone(), &stale_coverage).unwrap_err();
    assert_eq!(
        err,
        ConformanceContractError::ClaimObservationMismatch {
            domain: EvidenceDomain::Runtime,
            claimed: SupportObservationState::Observed,
            observed: SupportObservationState::Stale
        }
    );
    ...
    let set = ConformanceContractSet {
        contract_version: CONTRACT_VERSION,
        evaluated_at_ms: EVALUATED_AT,
        domain_coverage: stale_coverage.clone(),
        support_rows: rows.clone(),
    };
    let err = canonicalize_contract_set(set).unwrap_err();
    assert_eq!(
        err,
        ConformanceContractError::ClaimObservationMismatch {
            domain: EvidenceDomain::Runtime,
    ...
    assert_eq!(
        validate_conformance_contract_set(&ConformanceContractSet {
            contract_version: CONTRACT_VERSION,
            evaluated_at_ms: EVALUATED_AT,
            domain_coverage: stale_coverage,
            support_rows: rows.clone(),
        }),
        Err(ConformanceContractError::ClaimObservationMismatch {
            domain: EvidenceDomain::Runtime,
            claimed: SupportObservationState::Observed,
            observed: SupportObservationState::Stale
        })
    ...
    let mut expired_coverage = coverage();
    let runtime = expired_coverage
        .iter_mut()
        .find(|row| row.domain == EvidenceDomain::Runtime)
        .unwrap();
    ...
    let err =
        validate_capability_support_row_against_coverage(&rows[0], &expired_coverage).unwrap_err();
    assert_eq!(
        err,
        ConformanceContractError::DomainExpired {
            domain: EvidenceDomain::Runtime,
            expired_at_ms: EVALUATED_AT,
            evaluated_at_ms: EVALUATED_AT
        }
    );

    let err = canonicalize_support_claim_set(rows.clone(), &expired_coverage).unwrap_err();
    assert_eq!(
        err,
        ConformanceContractError::DomainExpired {
            domain: EvidenceDomain::Runtime,
    ...
    validate_support_claim_set(&rows)
        .expect("intrinsic validation remains independent of coverage expiry");
}
...
fn stale_dependency_invalidates_only_claims_that_require_it() {
    ...
    validate_capability_support_row_against_coverage(&verified_row(), &domains)
    ...
    assert!(matches!(
        validate_capability_support_row_against_coverage(&store_claim, &domains),
        Err(ConformanceContractError::InvalidSupportCombination { .. })
    ));
...
fn canonical_domain_helper_restores_order_but_not_duplicate_identity() {
    ...
    assert!(rows.iter().map(|row| row.domain).eq(EvidenceDomain::ALL));
