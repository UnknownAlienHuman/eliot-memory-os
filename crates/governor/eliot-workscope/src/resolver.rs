...
impl HostObservedHandles {
    ...
    pub fn validate(&self) -> Result<(), WorkScopeError> {
        ...
        for handle in self.open_file_refs.iter().chain(self.resource_refs.iter()) {
...
impl WorkScopeResolver {
        ...
        let matched = filter_candidates(set, |candidate| {
            candidate.instance.root_identity == handles.observed_root_ref
                || handles
                    .vcs_identity_ref
                    .as_deref()
                    .is_some_and(|vcs| candidate.instance.vcs_identity_ref.as_deref() == Some(vcs))
        });
        ResolutionOutcome {
    ...
    fn tier_registered(
        ...
        let matched = filter_candidates(set, |candidate| {
            registered.registered_instance_ref.as_deref()
                == Some(candidate.scope.instance_ref.as_str())
                || registered
                    .relocation_receipt
                    .as_ref()
                    .is_some_and(|receipt| {
