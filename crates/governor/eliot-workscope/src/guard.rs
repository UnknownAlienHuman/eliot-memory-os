...
pub fn check_at_trigger(
    ...
    let receipt = source_closure.map(|(sources, privacy)| {
        ScopeBindingGuard.check(expected, observed, sources, privacy)
    });
...
impl TriggerReport {
    ...
    pub fn validate(&self) -> Result<(), WorkScopeError> {
        if let Some(receipt) = &self.receipt {
            ...
            text(&receipt.expected_instance_ref, "report.expected_instance_ref")?;
            text(&receipt.observed_instance_ref, "report.observed_instance_ref")?;
