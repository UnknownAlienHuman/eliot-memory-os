...
pub use peer_communication::{
    ...
    MarkerDisposition, OPTIONAL_PEER_ENVELOPE_FIELDS, PEER_CHANNEL_REVISION, PEER_MESSAGE_SCHEMA,
    MAX_BOARD_ENTRIES_PER_SCOPE, MAX_BOARD_PAGE_SIZE, MAX_BOARD_REVISIONS_PER_ENTRY,
    MAX_PEER_ATTEMPT_HISTORY, MAX_PEER_INLINE_TEXT, MAX_PEER_MESSAGE_BYTES, MAX_PEER_OUTSTANDING_PER_RECIPIENT,
    MAX_PEER_OUTSTANDING_PER_SENDER, MAX_PEER_REFERENCES, MAX_PEER_STREAM_DEPTH,
    PeerAckReceipt, PeerArtifactHead, PeerAttemptRecord, PeerBoardKind, PeerClockPort,
    ...
    PeerDurabilityPort, PeerEndpointLossReport, PeerEnvelopeHeader, PeerMessage,
    PeerMessageDiagnostic, PeerMessageKind, PeerMessageState, PeerReconnectReport,
    PeerReviewAckReceipt, PeerReviewAdvance, PeerReviewDenominator, PeerReviewLifecycle,
    PeerReviewReceipt, PeerReviewStanding, PeerStreamHead, PeerStreamId, PostBoardEntry,
    PrivacyClass, RawField, RecordPeerConflict, REQUIRED_PEER_ENVELOPE_FIELDS,
...
impl CoordinationOwner {
    ...
    pub fn from_snapshot(snapshot: Self) -> Result<Self, CoordinationError> {
        if snapshot.events.iter().enumerate().any(|(index, event)| {
            event.sequence != index as u64 + 1
                ...
                || !event
                    .authority_epoch
                    .is_same_authority(&event.state_fence.authority_epoch)
        }) {
    ...
    fn validate_active_bindings(&self) -> Result<(), CoordinationError> {
        for (work_item_id, item) in self.work.iter().filter(|(_, item)| {
            ...
            if item.work_item_id != *work_item_id
                ...
                || !session
                    .authority_epoch
                    .is_same_authority(&item.state_fence.authority_epoch.clone())
    ...
    fn exact_work_replay(
        ...
        let exact_event = event.kind == CoordinationEventKind::WorkClaimed
            ...
            && event
                .authority_epoch
                .is_same_authority(&req.authority_epoch.clone())
...
mod tests {
    ...
    fn read_active_work_lease_returns_coherent_cloned_projection() {
        ...
        let projection = owner
            .read_active_work_lease("work-1", "session-1", 50, test_epoch(1), &state_fence)
    ...
    fn reads_reject_stale_epoch_and_fence() {
        ...
        let stale_fence = StateFence::new(test_epoch(1), ResourceGeneration::new(2).unwrap());
        assert_eq!(
        ...
        let stale_epoch_fence = StateFence::new(stale_epoch.clone(), ResourceGeneration::genesis());
        assert_eq!(
    ...
    fn active_session_heartbeat_validation_is_shared_by_reads_and_recovery() {
        ...
        assert_eq!(
            zero_deadline.read_active_session("session-1", 50, test_epoch(1), &state_fence,),
        ...
        assert!(matches!(
            CoordinationOwner::from_snapshot_at(zero_deadline, test_epoch(1), &state_fence,),
            Err(CoordinationError::InvalidField("heartbeat_deadline"))
        ));
    ...
    fn reads_reject_wrong_work_links_and_terminal_work() {
        ...
        assert!(matches!(
            owner.read_active_work_lease("work-1", "session-1", 50, test_epoch(1), &state_fence,),
    ...
    fn reassignment_retires_old_lease_and_preserves_one_active_binding() {
        ...
        let recovered = CoordinationOwner::from_snapshot_at(owner, test_epoch(1), &state_fence)
