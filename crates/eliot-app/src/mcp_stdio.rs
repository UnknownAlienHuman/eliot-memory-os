...
fn cognitive_policy_snapshot_id(input: &CognitiveSealInput) -> Result<String> {
    sha256_json(&(
...
fn validate_cognitive_source_binding(input: &CognitiveSealInput) -> Result<()> {
...
fn cognitive_exposure_sha256(call: &CognitiveRunCallPlan) -> Result<String> {
...
fn validate_cognitive_execution(execution: &CognitiveExecutionSeal) -> Result<()> {
    for (field, value) in [
...
fn validate_cognitive_plan(plan: &[CognitiveRunCallPlan]) -> Result<()> {
...
#[cfg(test)]
mod cognitive_contract_unit_tests {
    #![allow(clippy::expect_used)]
    use super::*;
    ...
    fn stdio_facade_rejects_inherited_database_credentials_by_name_only() {
        ...
        assert!(
            inherited_database_credential_variable([std::ffi::OsString::from("Path")]).is_none()
        );
    }
    ...
    async fn cognitive_state_machine_db_inner() -> Result<()> {
        ...
        let operator_agent_session =
            AgentSessionId::from_uuid(operator_context.session_id.as_uuid());
...
fn cognitive_revision_key(run_id: &str, revision: u64) -> String {
    format!("cognitive-run:{run_id}:revision:{revision}")
}

fn cognitive_tool_observation_subject(run_id: &str, call_number: u8) -> String {
    format!("{run_id}:call:{call_number}")
}

async fn cognitive_record_by_revision<T: serde::de::DeserializeOwned>(
    state: &McpState,
    project_id: ProjectId,
    task_id: TaskId,
    run_id: &str,
...
fn cognitive_commit_serializer() -> &'static tokio::sync::Mutex<()> {
...
fn deterministic_cognitive_uuid(material: &str) -> uuid::Uuid {
...
fn write_cognitive_capability_file(path: &Path, file: &CognitiveCapabilityFile) -> Result<()> {
    let parent = path
        .parent()
...
fn read_cognitive_capability_file(path: &Path) -> Result<CognitiveCapabilityFile> {
...
async fn validate_cognitive_gate(
    state: &McpState,
...
async fn load_cognitive_contract(
    state: &McpState,
...
#[allow(clippy::too_many_lines)]
async fn validate_cognitive_host_and_tools(
    state: &McpState,
...
async fn ensure_cognitive_raw_verifier(
    state: &McpState,
    context: AuthenticatedRequestContext,
    project_id: ProjectId,
    task_id: TaskId,
...
#[allow(clippy::too_many_lines)]
async fn cognitive_run_terminal(
    state: &McpState,
    context: AuthenticatedRequestContext,
    params: Value,
) -> Result<Value> {
...
#[allow(clippy::expect_used, clippy::too_many_lines)]
async fn cognitive_run_status(state: &McpState, params: Value) -> Result<Value> {
    state.ensure_schema().await?;
...
fn cognitive_role_allows(role: CognitiveInvocationRole, tool_name: &str) -> bool {
...
async fn cognitive_tool_definitions(state: &McpState, session_id: SessionId) -> Result<Vec<Value>> {
...
async fn dispatch_host_governor_method(
    state: &McpState,
    method: &str,
    params: Value,
) -> Option<Result<Value>> {
...
fn cognitive_memory_request_allowed(
    capability: &CognitiveCandidateCapability,
    tool_name: &str,
    arguments: &Value,
) -> bool {
...
fn restrict_cognitive_recall(structured: &mut Value, expected_handles: &[String]) -> Result<()> {
    let returned = {
...
async fn ensure_cognitive_tool_observation_capacity(
    state: &McpState,
    capability: &CognitiveCandidateCapability,
) -> Result<()> {
...
async fn write_cognitive_tool_observation(
    state: &McpState,
    context: AuthenticatedRequestContext,
...
async fn dispatch_cognitive_job_fetch(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
#[allow(clippy::too_many_lines)]
async fn dispatch_agent_candidate_submit(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
fn existing_candidate_with_same_topic_and_statement<'a>(
...
fn validate_candidate_field(name: &str, value: &str, max_bytes: usize) -> Result<()> {
...
fn validate_candidate_curation(curation: &AgentCandidateCurationInput) -> Result<()> {
...
fn finalize_verifier_scope_hash(scope: &mut VerifierArtifactScope) -> Result<()> {
    scope.canonical_scope_hash.clear();
    scope.canonical_scope_hash = canonical_struct_hash(scope)?;
    Ok(())
}

fn verifier_scope_hash_is_valid(scope: &VerifierArtifactScope) -> Result<bool> {
...
async fn run_registered_cargo_verifier(
    worktree: &Path,
    args: &[&str],
    timeout_seconds: u64,
    verifier_name: &str,
...
async fn run_dogfood_blob_verifier(worktree: &Path) -> Result<()> {
...
async fn run_cargo_workspace_check_verifier(worktree: &Path) -> Result<()> {
    run_registered_cargo_verifier(
        worktree,
        &[
            "check",
...
#[allow(clippy::too_many_lines)]
async fn resolve_verifier_artifact_scope(
    project_id: ProjectId,
    task: &TaskContract,
...
#[allow(clippy::too_many_lines)]
async fn dispatch_task_verification_run(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
fn registered_verifier_for_scope(scope: &VerifierArtifactScope) -> Option<RegisteredTaskVerifier> {
...
async fn revalidate_verifier_scope(
    state: &McpState,
    task: &TaskContract,
...
async fn dispatch_memory_corpus_profile(state: &McpState, arguments: Value) -> Result<Value> {
...
#[allow(clippy::too_many_lines)]
async fn dispatch_memory_curation_preview(state: &McpState, arguments: Value) -> Result<Value> {
...
fn remove_protected_curation_candidates(
...
fn curation_record_is_protected(record: &CanonicalRecord<Value>, metadata: Option<&Value>) -> bool {
...
fn explicit_curation_metadata(value: &Value) -> Option<&Value> {
    value
        .get("curation")
...
fn curation_string(value: &Value, name: &str) -> Option<String> {
    value
        .get(name)
        .and_then(Value::as_str)
        .map(str::trim)
...
fn curation_first_string(value: &Value, name: &str) -> Option<String> {
    value
        .get(name)
        .and_then(Value::as_array)
...
fn curation_strings(value: &Value, name: &str) -> Vec<String> {
...
fn curation_numeric(value: &Value, name: &str) -> Option<f64> {
...
async fn dispatch_cognitive_lab_evaluate(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
async fn dispatch_cognitive_failure_localization_record(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
fn verification_id_from_ref(reference: &str) -> Result<VerificationId> {
    let value = reference
        .trim()
...
fn canonical_verifier_matches_required(
    required: &str,
...
fn require_exact_canonical_verifier_set(
...
fn require_completion_proof_verifier_binding(
...
fn candidate_promotion_replay_matches(
...
async fn require_candidate_disposition_actor(
    state: &McpState,
    context: AuthenticatedRequestContext,
    task: &TaskContract,
...
async fn existing_candidate_promotion(
    state: &McpState,
...
fn task_verifier_artifacts(task: &TaskContract) -> Vec<VerifierArtifactRef> {
    task.verification_scopes
        .iter()
...
fn validate_procedure_candidate_source_refs(
    task: &TaskContract,
...
fn procedure_candidate_identity(
    task: &TaskContract,
    pattern: &ExperiencePattern,
...
fn procedure_candidate_response(
...
async fn persist_procedure_candidate(
    state: &McpState,
    context: AuthenticatedRequestContext,
    idempotency_key: &str,
    record: CanonicalProcedureSkillCandidate,
...
async fn dispatch_procedure_candidate_create(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
async fn resolve_procedure_candidate(
    state: &McpState,
    project_id: ProjectId,
    task_id: TaskId,
    candidate_ref: &str,
...
async fn dispatch_procedure_candidate_disposition(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
fn dispatch_eval_case_list(arguments: Value) -> Result<Value> {
...
fn dispatch_eval_suite_list(state: &McpState, arguments: Value) -> Result<Value> {
...
fn dispatch_eval_run(state: &McpState, arguments: Value) -> Result<Value> {
...
fn dispatch_eval_verdict(state: &McpState, arguments: Value) -> Result<Value> {
...
fn dispatch_eval_report(state: &McpState) -> Result<Value> {
    let artifacts = mcp_eval_artifacts(state, "k0-core-smoke", None)?;
    let report = json!({
...
fn dispatch_eval_smoke(state: &McpState, arguments: Value) -> Result<Value> {
    let input: EvalSuiteToolInput = serde_json::from_value(arguments)?;
    let artifacts = mcp_eval_artifacts(
        state,
        input.suite.as_deref().unwrap_or("k0-core-smoke"),
...
fn dispatch_eval_coverage(state: &McpState, arguments: Value) -> Result<Value> {
    let input: EvalSuiteToolInput = serde_json::from_value(arguments)?;
    let artifacts = mcp_eval_artifacts(
        state,
        input.suite.as_deref().unwrap_or("k0-core-smoke"),
...
fn dispatch_eval_baseline_list(state: &McpState, arguments: Value) -> Result<Value> {
    let input: EvalSuiteToolInput = serde_json::from_value(arguments)?;
    let artifacts = mcp_eval_artifacts(
        state,
        input.suite.as_deref().unwrap_or("k0-core-smoke"),
...
fn dispatch_eval_compare(state: &McpState, arguments: Value) -> Result<Value> {
...
fn dispatch_eval_gate(state: &McpState, arguments: Value) -> Result<Value> {
...
fn dispatch_eval_profiles(state: &McpState) -> Result<Value> {
...
fn dispatch_eval_trend(state: &McpState, arguments: Value) -> Result<Value> {
    let input: EvalSuiteToolInput = serde_json::from_value(arguments)?;
    let artifacts = mcp_eval_artifacts(
        state,
        input.suite.as_deref().unwrap_or("k0-core-smoke"),
...
fn dispatch_verify_profiles(state: &McpState) -> Result<Value> {
    let report = json!({
...
fn dispatch_verify_inventory(state: &McpState) -> Result<Value> {
...
fn dispatch_verify_plan(state: &McpState, arguments: Value) -> Result<Value> {
...
fn dispatch_verify_report(state: &McpState) -> Result<Value> {
    let inventory = mcp_verification_inventory();
...
fn dispatch_verify_cost_report(state: &McpState) -> Result<Value> {
    let inventory = mcp_verification_inventory();
...
fn dispatch_verify_last_verdict(state: &McpState) -> Result<Value> {
...
fn dispatch_metrics_registry(state: &McpState) -> Result<Value> {
    serde_json::to_value(mcp_metrics_registry_report(state)?).map_err(Into::into)
}

fn dispatch_metrics_dashboard(state: &McpState) -> Result<Value> {
    serde_json::to_value(mcp_metrics_dashboard_report(state)?).map_err(Into::into)
}

fn dispatch_metrics_slo(state: &McpState) -> Result<Value> {
...
fn dispatch_metrics_latency(state: &McpState) -> Result<Value> {
    let samples = mcp_metrics_samples_report(state)?.samples;
    let rollup = mcp_metrics_rollup_report(state, &samples)?.rollup;
    serde_json::to_value(mcp_metrics_latency_report(state, &rollup)?).map_err(Into::into)
}

fn dispatch_metrics_cost(state: &McpState) -> Result<Value> {
    serde_json::to_value(mcp_metrics_cost_report(state)?).map_err(Into::into)
}

fn dispatch_metrics_quality(state: &McpState) -> Result<Value> {
    serde_json::to_value(mcp_metrics_quality_report(state)?).map_err(Into::into)
}

fn dispatch_metrics_report(state: &McpState) -> Result<Value> {
...
fn mcp_metrics_summary_report(state: &McpState) -> Result<Value> {
...
fn mcp_metrics_dashboard_report(state: &McpState) -> Result<DashboardReport> {
...
fn mcp_recent_incident_refs(state: &McpState) -> Vec<String> {
    if state
        .root
        .join("reports")
        .join("incident")
...
fn mcp_verification_inventory() -> TestInventory {
    TestInventoryService.generate(project_id_from_label("eliot-governor"))
}

#[allow(clippy::too_many_arguments)]
fn write_mcp_verification_artifacts(
    state: &McpState,
...
fn mcp_latest_verification_run(root: &Path) -> Result<ProfileVerificationRun> {
...
fn write_verification_report_json_md<T: serde::Serialize>(
    state: &McpState,
    dir: &str,
    title: &str,
    value: &T,
...
fn write_eval_report_json_md<T: serde::Serialize>(
    state: &McpState,
    dir: &str,
    title: &str,
    value: &T,
...
async fn dispatch_trace_completeness(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
fn maybe_inject_m2_failure(variable: &str, key: &str, checkpoint: &str) -> Result<()> {
...
fn validate_canonical_replay_request(
...
async fn persist_l11_record<T: serde::Serialize>(
    state: &McpState,
    context: AuthenticatedRequestContext,
    project_id: ProjectId,
    task_id: TaskId,
...
async fn reconcile_l11_record<T>(
    state: &McpState,
    context: AuthenticatedRequestContext,
...
fn dispatch_replay_report(state: &McpState) -> Value {
    json!({
...
fn finalize_sleep_bundle_identity(bundle: &mut SleepConsolidationBundle) -> Result<()> {
...
fn validate_sleep_bundle(bundle: &SleepConsolidationBundle) -> Result<()> {
...
async fn reconcile_sleep_artifacts(
    state: &McpState,
    context: AuthenticatedRequestContext,
    project_id: ProjectId,
    task_id: TaskId,
...
fn maybe_inject_sleep_reconciliation_failure(completed: usize) -> Result<()> {
    if cfg!(debug_assertions)
...
const fn sleep_artifact_receipt_kind(kind: SleepCandidateArtifactKind) -> CanonicalReceiptKind {
    match kind {
...
fn validate_sleep_artifact(artifact: &eliot_types::SleepCandidateArtifact) -> Result<()> {
...
#[allow(clippy::too_many_lines)]
async fn dispatch_meta_experiment_run(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
async fn reconcile_authoritative_meta_experiment(
    state: &McpState,
    context: AuthenticatedRequestContext,
...
fn require_receipted_meta_rejection(has_rejection: bool, has_receipt: bool) -> Result<()> {
...
const fn meta_change_class_supported(change_class: MetaCandidateChangeClass) -> bool {
    matches!(change_class, MetaCandidateChangeClass::VerificationMap)
}

async fn canonical_execution_by_id(
    state: &McpState,
    project_id: ProjectId,
    task_id: TaskId,
    execution_id: &str,
...
fn m2_meta_commit_serializer() -> &'static tokio::sync::Mutex<()> {
    M2_META_COMMIT_SERIALIZER.get_or_init(|| tokio::sync::Mutex::new(()))
}

async fn revalidate_execution_trace_authority(
    state: &McpState,
    task: &TaskContract,
...
async fn revalidate_meta_experiment_trace_authority(
    state: &McpState,
    task: &TaskContract,
    experiment: &eliot_types::HarnessExperimentRecord,
) -> Result<()> {
...
async fn canonical_set_for_execution(
    state: &McpState,
    project_id: ProjectId,
    task_id: TaskId,
...
fn meta_policy_action_replay_result(
...
async fn rehydrate_meta_policy_action_replay(
    state: &McpState,
    context: AuthenticatedRequestContext,
    project_id: ProjectId,
    task_id: TaskId,
...
async fn canonical_meta_experiment(
    state: &McpState,
    project_id: ProjectId,
    task_id: TaskId,
    experiment_id: &str,
...
async fn current_meta_policy_candidate(
    state: &McpState,
    project_id: ProjectId,
    task_id: TaskId,
    experiment: &eliot_types::HarnessExperimentRecord,
...
async fn canonical_meta_metrics(
    state: &McpState,
    project_id: ProjectId,
    task_id: TaskId,
    experiment: &eliot_types::HarnessExperimentRecord,
...
async fn canonical_meta_rejections(
    state: &McpState,
    project_id: ProjectId,
    task_id: TaskId,
    experiment_id: &str,
...
async fn canonical_meta_dispositions(
    state: &McpState,
    project_id: ProjectId,
    task_id: TaskId,
    experiment_id: &str,
...
#[allow(clippy::too_many_lines)]
async fn dispatch_meta_experiment_disposition(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
async fn reject_distinct_meta_terminal_action(
    state: &McpState,
    project_id: ProjectId,
    task_id: TaskId,
    candidate_id: &str,
...
async fn dispatch_l11_status(state: &McpState, arguments: Value) -> Result<Value> {
...
fn complete_authoritative_replay_executions(
...
fn dispatch_memory_lifecycle_status(arguments: Value) -> Result<Value> {
...
fn dispatch_memory_lifecycle_propose(arguments: Value) -> Result<Value> {
...
fn dispatch_memory_lifecycle_vitality(arguments: Value) -> Result<Value> {
...
fn dispatch_memory_lifecycle_gravity(arguments: Value) -> Result<Value> {
    let input: MemoryLifecycleProjectToolInput = serde_json::from_value(arguments)?;
    let score = MemoryVitalityService::score(
        project_id_from_label(&input.project),
        input
...
async fn dispatch_memory_lifecycle_influence(state: &McpState, arguments: Value) -> Result<Value> {
...
fn dispatch_skill_list(arguments: Value) -> Result<Value> {
...
fn dispatch_skill_inspect(arguments: Value) -> Result<Value> {
...
fn dispatch_skill_estimate(arguments: Value) -> Result<Value> {
...
fn dispatch_skill_filter(arguments: Value) -> Result<Value> {
    let input: SkillTaskToolInput = serde_json::from_value(arguments)?;
    let skills = vec![
        mcp_active_skill(SkillId::new_v7(), SkillLifecycleState::Active),
        SkillRegistryService::create_candidate("MCP skill candidate", "mcp"),
...
fn dispatch_skill_influence(arguments: Value) -> Result<Value> {
    let input: SkillTaskToolInput = serde_json::from_value(arguments)?;
    let skill = mcp_active_skill(SkillId::new_v7(), SkillLifecycleState::Active);
...
async fn dispatch_skill_execution_proof(state: &McpState, arguments: Value) -> Result<Value> {
...
async fn dispatch_skill_create_candidate(state: &McpState, arguments: Value) -> Result<Value> {
...
async fn dispatch_skill_curator_run(state: &McpState, arguments: Value) -> Result<Value> {
...
async fn dispatch_skill_curator_proposals(state: &McpState, arguments: Value) -> Result<Value> {
    let input: SkillProjectToolInput = serde_json::from_value(arguments)?;
...
fn dispatch_skill_curator_inspect(state: &McpState, arguments: Value) -> Result<Value> {
...
fn dispatch_skill_curator_report(state: &McpState) -> Result<Value> {
    let report = latest_json_report(
        &state
            .root
            .join("reports")
...
fn dispatch_skill_curator_gate(state: &McpState, arguments: Value) -> Result<Value> {
...
async fn dispatch_action_plan(state: &McpState, arguments: Value) -> Result<Value> {
...
fn dispatch_action_lease_status(state: &McpState, arguments: Value) -> Result<Value> {
...
async fn dispatch_patch_preflight(state: &McpState, arguments: Value) -> Result<Value> {
...
async fn dispatch_patch_apply(state: &McpState, arguments: Value) -> Result<Value> {
    let input: PatchToolInput = serde_json::from_value(arguments)?;
    let blob_store = BlobStore::open(&state.blob_store)?;
    let (request, lease, report, verifier_plan) =
        patch_request_from_input(&state.root, &input.lease_id, input.diff_text)?;
...
fn dispatch_patch_status(state: &McpState, arguments: Value) -> Result<Value> {
...
fn dispatch_verifier_status(state: &McpState, arguments: Value) -> Result<Value> {
...
async fn dispatch_work_create(state: &McpState, arguments: Value) -> Result<Value> {
...
async fn dispatch_work_claim(state: &McpState, arguments: Value) -> Result<Value> {
...
fn dispatch_work_status(state: &McpState, arguments: Value) -> Result<Value> {
...
async fn dispatch_work_renew(state: &McpState, arguments: Value) -> Result<Value> {
...
async fn dispatch_work_release(state: &McpState, arguments: Value) -> Result<Value> {
    let input: WorkLeaseToolInput = serde_json::from_value(arguments)?;
    let mut work_state = load_work_state(&state.root)?;
    let lease_id = WorkLeaseId::from_str(&input.lease_id).context("parse work lease id")?;
...
fn dispatch_work_conflicts(state: &McpState, arguments: Value) -> Result<Value> {
    let input: WorkStatusToolInput = serde_json::from_value(arguments)?;
    let work_state = load_work_state(&state.root)?;
    let report = WorkQueueService.status_report(&work_state, &input.project, &input.task);
    Ok(json!({
...
async fn dispatch_worktree_create(state: &McpState, arguments: Value) -> Result<Value> {
...
fn dispatch_worktree_status(state: &McpState, arguments: Value) -> Result<Value> {
...
async fn dispatch_worktree_capture_diff(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
async fn require_current_candidate_reviewer(
    state: &McpState,
    context: AuthenticatedRequestContext,
...
async fn dispatch_worktree_review(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
async fn dispatch_worktree_cleanup(state: &McpState, arguments: Value) -> Result<Value> {
    let input: WorktreeLeaseToolInput = serde_json::from_value(arguments)?;
    let mut work_state = load_work_state(&state.root)?;
    let worktree_lease_id =
        WorktreeLeaseId::from_str(&input.worktree_lease).context("parse worktree lease id")?;
...
fn host_id_from_client_kind(client_kind: &str) -> Option<AgentHostId> {
    match client_kind {
...
fn dispatch_host_session_status(
    state: &McpState,
    context: AuthenticatedRequestContext,
) -> Result<Value> {
...
fn authorize_dynamic_delegation(
    state: &McpState,
    context: AuthenticatedRequestContext,
...
async fn dispatch_agent_delegate(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
async fn dispatch_agent_result_submit(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
fn append_candidate_diff_authority_ref(root: &Path, artifact_refs: &mut Vec<String>) -> Result<()> {
...
async fn dispatch_agent_result_disposition(
    state: &McpState,
    context: AuthenticatedRequestContext,
    arguments: Value,
) -> Result<Value> {
...
fn require_controller_capability(
    broker_state: &eliot_types::DelegationState,
    agent_session_id: AgentSessionId,
    task_id: TaskId,
    capability: &str,
...
fn validate_broker_text(field: &str, value: &str, max_len: usize) -> Result<()> {
...
fn validate_broker_refs(field: &str, values: &[String]) -> Result<()> {
    if values.len() > 64
        || values
            .iter()
...
fn cognitive_record_schema(field: &str) -> Value {
...
fn verifier_latest_path(root: &Path) -> PathBuf {
...
fn candidate_diff_report_markdown(report: &Value) -> String {
    format!(
...
fn validate_managed_verification_run_identity(
    run: &VerificationRun,
...
#[allow(clippy::too_many_lines)]
async fn validate_managed_actual_verifier_refs(
    state: &McpState,
    managed: &crate::host_runtime::ManagedControllerCandidate,
...
fn materialize_managed_candidate(
    state: &McpState,
...
async fn canonicalize_candidate_artifacts(
    state: &McpState,
    context: AuthenticatedRequestContext,
    intent: &ManagedFinalizationIntent,
...
fn managed_candidate_file_sets(candidate: &[u8]) -> Result<ManagedCandidateFileSets> {
...
fn apply_candidate_diff(worktree: &Path, candidate: &[u8]) -> Result<()> {
...
fn managed_candidate_hash(bytes: &[u8]) -> String {
...
fn ensure_managed_candidate_commit(
    worktree: &Path,
    intent: &ManagedFinalizationIntent,
    candidate: &[u8],
) -> Result<String> {
...
fn commit_candidate_diff(worktree: &Path, intent: &ManagedFinalizationIntent) -> Result<String> {
...
mod l4_tests {
    ...
    fn legacy_finalization_records_without_verifier_refs_fail_deserialization() -> Result<()> {
        ...
        let aggregate_error =
            serde_json::from_value::<ManagedFinalizationAggregate>(legacy_aggregate)
                .err()
    ...
    fn finalization_git_fixture(
        label: &str,
    ) -> Result<(PathBuf, Vec<u8>, ManagedFinalizationIntent)> {
    ...
    fn operator_candidate_evidence_must_cover_exact_source_provenance() -> Result<()> {
        ...
        assert!(
            require_operator_candidate_evidence(
                &candidate,
                &["unrelated-verifier:passed".to_owned()]
            )
            .is_err()
        );
        assert_eq!(
    ...
    fn operator_memory_projection_exposes_rank_suppression_and_exact_resolution_fields() {
        ...
        assert!(trace.fields.iter().any(|field| {
            field.label == "query_mode"
                && field.value == "query_aware_semantic_lexical_relational_v2"
        }));
        ...
        assert!(l2_records.iter().any(|record| {
            record.record_ref == format!("claim:{active_id}")
                && record.fields.iter().any(|field| {
                    field.label == "payload_json" && field.value.contains("receipt:exact")
                })
        }));
    }
    ...
    fn operator_selected_graph_neighborhood_is_bounded_and_preserves_provenance() {
        ...
        let selected =
            operator_selected_neighborhood(vec![first, disconnected], Some("claim:1"), 1);
    ...
    fn m3_terminal_authority_is_one_atomic_proof_bound_aggregate() -> Result<()> {
        ...
        let decoded = decode_authoritative_autonomy_aggregate(&record)?
            .context("valid terminal aggregate")?;
    ...
    fn l12_provider_free_one_drive_source_finalizes_only_in_local_app_data() -> Result<()> {
        ...
        let baseline = String::from_utf8(git(&worktree, &["rev-parse", "HEAD"])?.stdout)?
            ...
            .to_owned();
        let intent =
            finalization_intent("local-appdata-authority", baseline, &diff, "fixture.txt")?;
