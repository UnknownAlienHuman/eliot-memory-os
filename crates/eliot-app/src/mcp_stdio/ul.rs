...
impl UlRuntime {
    ...
    pub async fn effective_injection_mode(
        ...
        // The hidden A/B arm is measurement metadata. Only an explicit
        // certification control request may suppress production injection.
        let mode = if explicit_control {
            None
        } else if let Some(assignment) = assignment.as_ref() {
            self.production_injection_mode(assignment).await?
        } else {
