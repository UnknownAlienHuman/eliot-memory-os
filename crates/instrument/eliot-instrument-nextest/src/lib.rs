...
impl NextestCommand {
    ...
    pub fn run(target: impl Into<String>, profile: impl Into<String>, filters: &[String]) -> Result<Self, NextestError> {
        let target = checked_text(target.into(), "target")?;
        let profile = checked_text(profile.into(), "profile")?;
        let mut arguments = vec!["nextest".to_owned(), "run".to_owned(), "--profile".to_owned(), profile.clone()];
        for filter in filters {
            arguments.push(checked_text(filter.clone(), "filter")?);
        }
        Ok(Self { executable: "cargo".to_owned(), arguments, target, profile })
    }
...
pub fn parse_jsonl(bytes: &[u8]) -> Result<NextestReport, NextestError> {
    ...
    for line in bytes.split(|byte| *byte == b'\n') {
        ...
        let event: JsonEvent = serde_json::from_str(line).map_err(|_| NextestError::MalformedEvent)?;
        ...
        if matches!(event_name, "started" | "STARTED") {
            ...
            report.started = report.started.checked_add(1).ok_or(NextestError::CounterOverflow)?;
            continue;
        }
        ...
        match status.as_str() {
            "PASS" | "pass" => { report.passed = report.passed.checked_add(1).ok_or(NextestError::CounterOverflow)?; report.completed = report.completed.checked_add(1).ok_or(NextestError::CounterOverflow)?; }
            "FAIL" | "fail" | "XPASS" | "xpass" => { report.failed = report.failed.checked_add(1).ok_or(NextestError::CounterOverflow)?; report.completed = report.completed.checked_add(1).ok_or(NextestError::CounterOverflow)?; }
            "SKIP" | "skip" | "XFAIL" | "xfail" => { report.skipped = report.skipped.checked_add(1).ok_or(NextestError::CounterOverflow)?; report.completed = report.completed.checked_add(1).ok_or(NextestError::CounterOverflow)?; }
            "TIMEOUT" | "timeout" => { report.timed_out = report.timed_out.checked_add(1).ok_or(NextestError::CounterOverflow)?; report.completed = report.completed.checked_add(1).ok_or(NextestError::CounterOverflow)?; }
            "LEAK" | "leak" => { report.leaked = report.leaked.checked_add(1).ok_or(NextestError::CounterOverflow)?; report.completed = report.completed.checked_add(1).ok_or(NextestError::CounterOverflow)?; }
            "CANCEL" | "cancel" | "CANCELLED" | "cancelled" => { report.cancelled = report.cancelled.checked_add(1).ok_or(NextestError::CounterOverflow)?; report.completed = report.completed.checked_add(1).ok_or(NextestError::CounterOverflow)?; }
...
pub struct NextestAdapter<E> { executor: Arc<E> }
...
impl<E: ProcessExecutor + 'static> NextestAdapter<E> {
    ...
    pub async fn launch(
        ...
        invocation.validate().map_err(|error| NextestError::Invocation(error.to_string()))?;
        if invocation.kind != InstrumentKind::Test || invocation.instrument.to_string() != NEXTEST_INSTRUMENT {
    ...
    pub async fn inspect(&self, operation: &eliot_process::OperationId) -> Result<ProcessExecutionView, NextestError> {
        Ok(self.executor.inspect(operation.clone()).await?)
    }

    pub async fn cancel(&self, operation: &eliot_process::OperationId) -> Result<CancellationReceipt, NextestError> {
        Ok(self.executor.cancel(operation.clone()).await?)
    }

    pub async fn reconcile(&self, operation: &eliot_process::OperationId) -> Result<ProcessEvidence, NextestError> {
...
struct JsonEvent {
    ...
    #[serde(default)] event: Option<String>,
    #[serde(default)] name: Option<String>,
    #[serde(default)] status: Option<String>,
    #[serde(flatten)] _extra: std::collections::BTreeMap<String, Value>,
}

#[derive(Debug, Error)]
pub enum NextestError {
    #[error("invocation rejected: {0}")] Invocation(String),
    #[error("wrong instrument or invocation kind")] WrongInstrument,
    #[error("process command does not match admitted nextest command")] CommandMismatch,
    #[error("process receipt does not bind to the admitted request")] ReceiptMismatch,
    #[error("nextest output exceeds the bounded capture limit")] OutputTooLarge,
    #[error("nextest output contains an oversized line")] LineTooLarge,
    #[error("nextest output is not valid JSONL")] MalformedEvent,
    #[error("nextest emitted a duplicate test event")] DuplicateEvent,
    #[error("nextest emitted an unsupported status: {0}")] UnsupportedStatus(String),
    #[error("nextest counter overflowed")] CounterOverflow,
    #[error(transparent)] Process(#[from] ProcessExecutionError),
}

fn checked_text(value: String, field: &'static str) -> Result<String, NextestError> {
    if value.trim().is_empty() || value.chars().any(char::is_control) { return Err(NextestError::Invocation(format!("{field} must be non-blank and free of control characters"))); }
    Ok(value)
}

fn trim_utf8(bytes: &[u8]) -> Result<&str, NextestError> {
    std::str::from_utf8(bytes).map(str::trim).map_err(|_| NextestError::MalformedEvent)
}
