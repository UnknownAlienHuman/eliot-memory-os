...
fn committed_erasure_receipt() -> eliot_store_api::WriteReceipt {
    ...
    // Issue #18: the admission digest covers the canonical hash, so rebind
    // after the hash is rebound (erasure binds no source heads).
    eliot_store_api::bind_issue18_digests(&mut prepared, Vec::new()).expect("fixture digests bind");
