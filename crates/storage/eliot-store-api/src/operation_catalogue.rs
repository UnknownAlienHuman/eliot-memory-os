...
/// `RecordAuthorityFanoutState` persist `ReversibleMutation` through the
...
/// contract). All activated mutations address no scope, mirroring the
/// scope-free read descriptors. Every other mutation stays known-but-unsupported.
