...
pub(crate) const READ_GENESIS_SCHEMA_AND_STATE: &str = "BEGIN TRANSACTION; SELECT * FROM ONLY schema_meta:current; SELECT VALUE body FROM ONLY canonical_fence:current; SELECT VALUE { namespace: namespace, key: key, state_fence: state_fence, revision: revision, schema: schema, payload: payload, value_digest: value_digest } FROM recovery_owner; SELECT VALUE { namespace: namespace, key: key, state_fence: state_fence, revision: revision, schema: schema, payload: payload, value_digest: value_digest } FROM recovery_job; SELECT VALUE body FROM write_receipt; SELECT VALUE body FROM revision_head; SELECT VALUE body FROM ordering_head; SELECT VALUE body FROM canonical_event; SELECT VALUE body FROM projection_record; SELECT VALUE body FROM outbox_event; SELECT VALUE body FROM relation_record; COMMIT TRANSACTION;";

pub(crate) const TX_GENESIS_BEGIN: &str = "BEGIN TRANSACTION;";
pub(crate) const TX_GENESIS_SCHEMA_GUARD: &str = "LET $genesis_schema = (SELECT * FROM ONLY schema_meta:current); LET $genesis_fence = (SELECT VALUE body FROM ONLY canonical_fence:current); IF !type::is_object($genesis_schema) OR !type::is_object($genesis_fence) { THROW 'genesis_state_conflict'; }; IF $genesis_schema.generation != $expected_generation OR $genesis_fence.state_fence != $expected_state_fence OR $genesis_fence.next_commit_sequence != 1 OR $genesis_fence.next_outbox_sequence != 1 { THROW 'genesis_fence_conflict'; };";
