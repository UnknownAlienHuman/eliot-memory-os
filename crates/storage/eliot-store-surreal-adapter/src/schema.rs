...
/// Fenced upsert of the Governor Authority-owner image. The image is opaque
/// to the Store; this statement arbitrates only its fixed address, fence, and
/// owner revision so a missing/stale derivative projection cannot be replaced
/// with an unproven default.
pub(crate) const TX_AUTHORITY_OWNER: &str = "LET $authority_existing = (SELECT VALUE { namespace: namespace, key: key, state_fence: state_fence, revision: revision, value_digest: value_digest } FROM ONLY type::record($authority_owner_table, $authority_owner_id)); IF type::is_object($authority_existing) { LET $authority_owner_cas = (UPDATE type::record($authority_owner_table, $authority_owner_id) CONTENT $authority_owner_record WHERE state_fence = $authority_expected_state_fence AND revision = $authority_expected_revision AND value_digest = $authority_expected_digest RETURN AFTER); IF array::len($authority_owner_cas ?? []) != 1 { THROW 'authority_owner_cas_conflict'; }; } ELSE { IF $authority_expected_revision != 0 { THROW 'authority_owner_create_conflict'; }; LET $authority_owner_create = (CREATE type::record($authority_owner_table, $authority_owner_id) CONTENT $authority_owner_record RETURN AFTER); IF array::len($authority_owner_create ?? []) != 1 { THROW 'authority_owner_create_conflict'; }; };";
...
/// adapter arbitrates its stable closure key, fence, and exact prior digest.
pub(crate) const TX_AUTHORITY_REVOCATION: &str = "LET $revocation_existing = (SELECT VALUE { state_fence: state_fence, revision: revision, value_digest: value_digest } FROM ONLY type::record($revocation_owner_table, $revocation_owner_id)); IF type::is_object($revocation_existing) { IF $revocation_existing.state_fence != $revocation_expected_state_fence OR $revocation_existing.revision != $revocation_expected_revision OR $revocation_existing.value_digest != $revocation_expected_record_digest { THROW 'revocation_record_conflict'; }; } ELSE { IF $revocation_expected_revision != 0 { THROW 'revocation_record_create_conflict'; }; CREATE type::record($revocation_owner_table, $revocation_owner_id) CONTENT $revocation_owner_record; };";

/// Fenced CAS of the single Store-owned authority-revocation history root.
/// The root is written in the same transaction as the immutable record, so
/// a receipt can never exist with an unadvanced or forked history watermark.
pub(crate) const TX_AUTHORITY_REVOCATION_ROOT: &str = "LET $revocation_root_existing = (SELECT VALUE { state_fence: state_fence, revision: revision, value_digest: value_digest } FROM ONLY type::record($revocation_root_table, $revocation_root_id)); IF type::is_object($revocation_root_existing) { IF $revocation_root_existing.state_fence != $revocation_root_expected_state_fence OR $revocation_root_existing.revision != $revocation_root_expected_revision OR $revocation_root_existing.value_digest != $revocation_root_expected_digest { THROW 'revocation_root_cas_conflict'; }; LET $revocation_root_cas = (UPDATE type::record($revocation_root_table, $revocation_root_id) CONTENT $revocation_root_record WHERE state_fence = $revocation_root_expected_state_fence AND revision = $revocation_root_expected_revision AND value_digest = $revocation_root_expected_digest RETURN AFTER); IF array::len($revocation_root_cas ?? []) != 1 { THROW 'revocation_root_cas_conflict'; }; } ELSE { IF $revocation_root_expected_revision != 0 { THROW 'revocation_root_create_conflict'; }; LET $revocation_root_create = (CREATE type::record($revocation_root_table, $revocation_root_id) CONTENT $revocation_root_record RETURN AFTER); IF array::len($revocation_root_create ?? []) != 1 { THROW 'revocation_root_create_conflict'; }; };";
