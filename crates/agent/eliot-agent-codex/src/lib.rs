...
use eliot_agent_api::{
    ...
    UnsupportedEventReason, UsageReceipt, WorkLeaseId, contains_restricted_source_token,
};
use eliot_process::{
...
/// Omitted-manifest entry naming the decoded wire message body withheld
/// behind the restricted handle. Every decoded normalization seals a typed
/// summary projection, never the message itself, so the canonical body is
/// always partially omitted; the normalizer declares it instead of sealing
/// an empty claim.
pub const CODEX_MESSAGE_BODY_ENTRY: &str = "codex.message_body";
/// Omitted-manifest entry naming wire bytes that could not be decoded into a
/// typed observation.
pub const CODEX_UNDECODABLE_SOURCE_ENTRY: &str = "codex.source_bytes";
/// Warning carried when a delta/reasoning summary exceeds the bounded
/// public-summary capacity: the count stays exact, the over-cap remainder is
/// recorded as truncated normalization coverage, never silently dropped.
pub const CODEX_DELTA_TRUNCATED_WARNING: &str = "codex.delta-summary-truncated";

/// Splits a delta/reasoning character count into the exact count plus whether
/// the source text exceeds the bounded public-summary capacity
/// ([`MAX_HOST_EVENT_SAFE_TEXT_CHARS`]). Over-cap text is recorded with
/// [`NormalizationCoverage::TruncatedSource`]: the count stays exact, the
/// full text stays behind the restricted handle, and the over-cap remainder
/// is never silently dropped (I10.8.5). The observation `truncated` flag is
/// left untouched: it reports truncation before the restricted source
/// record, while the coverage reports the normalization's bounded summary
/// of a complete record.
fn bounded_delta_chars(params: &Value, keys: &[&str]) -> (u64, bool) {
    let chars = delta_chars_from(params, keys);
    let truncated = chars > MAX_HOST_EVENT_SAFE_TEXT_CHARS as u64;
    (chars, truncated)
}

/// Merges the decoded-message discovery into a classified payload: the
/// withheld canonical body joins the loss manifest (sorted, deduplicated)
/// and the coverage degrades from `Complete` to lossy omission. Quarantine
/// and already-lossy classifications keep their coverage; only an empty
/// `Complete` claim degrades.
fn merge_codex_discovery(
    mut omitted_fields: Vec<String>,
    coverage: NormalizationCoverage,
) -> (Vec<String>, NormalizationCoverage) {
    if !omitted_fields
        .iter()
        .any(|entry| entry == CODEX_MESSAGE_BODY_ENTRY)
    {
        omitted_fields.push(CODEX_MESSAGE_BODY_ENTRY.to_owned());
    }
    omitted_fields.sort();
    omitted_fields.dedup();
    let coverage = if coverage == NormalizationCoverage::Complete {
        NormalizationCoverage::LossyOmission
    } else {
        coverage
    };
    (omitted_fields, coverage)
}

/// Scans every wire-derived public string for restricted source content
/// before sealing. Raw provider text stays behind the restricted handle
/// (I7.23); a derived summary that smuggles it fails closed here.
fn check_codex_public_strings(
    payload: &NormalizedHostEventPayload,
...
/// Exact `"completed"`/`"success"`/`"done"` become observed terminal
/// success and `"failed"` becomes observed terminal failure, always under
/// the exact execution-unit lineage enforced by the caller. Every other
/// status (interrupted, unknown, in-progress, numeric, missing) stays
/// quarantine and never upgrades to success.
...
/// Effective Codex normalization source: the qualified digest plus whether
/// the raw bytes decoded to the supplied wire message.
enum BoundCodexSource {
    /// The raw bytes decoded to a wire message identical (canonically) to
    /// the supplied message; the digest covers the decoded canonical bytes.
    Decoded(QualifiedSourceDigest),
    /// The raw bytes did not decode; the digest covers the exact bytes under
    /// the raw-bytes qualifier and the supplied classification is replaced
    /// by typed quarantine.
    Undecodable(QualifiedSourceDigest),
}

/// Binds the Codex normalization source (P2 source-integrity repair): the
/// raw bytes must decode to exactly one wire message whose canonical form
/// equals the supplied message, or no receipt is issued. A completed-turn
/// message paired with failed-turn bytes (or any other disagreeing pair)
/// fails here as mismatched input, never sealed. Undecodable bytes never
/// mint the supplied classification: they bind the exact bytes under the
/// raw-bytes qualifier for the typed quarantine path.
fn bind_codex_source(
    message: &CodexWireMessage,
    raw_source_bytes: &[u8],
) -> Result<BoundCodexSource, CodexAdapterError> {
    let Ok(decoded) = CodexWireMessage::parse_line(raw_source_bytes) else {
        return Ok(BoundCodexSource::Undecodable(QualifiedSourceDigest {
            algorithm: HOST_EVENT_RAW_BYTES_DIGEST_ALGORITHM.to_owned(),
            digest: qualified_codex_digest(eliot_contracts::sha256_hex(raw_source_bytes))?,
        }));
    };
    let decoded_canonical = eliot_contracts::canonical_json_bytes(&decoded)
        .map_err(|_| CodexAdapterError::Contract(eliot_agent_api::ContractError::DigestMismatch))?;
    let supplied_canonical = eliot_contracts::canonical_json_bytes(message)
        .map_err(|_| CodexAdapterError::Contract(eliot_agent_api::ContractError::DigestMismatch))?;
    if decoded_canonical != supplied_canonical {
        return Err(CodexAdapterError::InvalidInput("raw_source_bytes/message"));
    }
    Ok(BoundCodexSource::Decoded(QualifiedSourceDigest {
        algorithm: HOST_EVENT_DIGEST_ALGORITHM.to_owned(),
        digest: qualified_codex_digest(eliot_contracts::sha256_hex(&decoded_canonical))?,
    }))
}

/// Builds a typed codex digest from a hex string.
fn qualified_codex_digest(hex: String) -> Result<LowercaseSha256, CodexAdapterError> {
    serde_json::from_value(Value::String(hex))
        .map_err(|_| CodexAdapterError::Contract(eliot_agent_api::ContractError::DigestMismatch))
}

/// Builds the typed quarantine for undecodable Codex bytes: the exact bytes
/// stay behind the restricted handle under the raw-bytes digest, nothing is
/// fabricated from them, and the actual omission is declared.
fn undecodable_codex_quarantine() -> ClassifiedCodexPayload {
    ClassifiedCodexPayload {
        payload: NormalizedHostEventPayload::UnsupportedQuarantined(UnsupportedEventObservation {
            source_namespace: CODEX_NORMALIZER_IDENTITY.to_owned(),
            source_version: None,
            reason: UnsupportedEventReason::SourceDecodeFailure,
            detail_ref: None,
        }),
        omitted_fields: vec![CODEX_UNDECODABLE_SOURCE_ENTRY.to_owned()],
        warnings: Vec::new(),
        privacy_class: HostEventPrivacyClass::RestrictedHandleOnly,
        coverage: NormalizationCoverage::LossyOmission,
    }
