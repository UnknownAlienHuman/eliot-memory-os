...
fn ordered_models(models: &BTreeMap<String, ProviderModel>) -> Vec<(&String, &ProviderModel)> {
...
fn compile_provider_entries(
    ...
    models: &BTreeMap<String, ProviderModel>,
    policy: &OpenCodeProviderRoutePolicy,
) -> Result<(Vec<ModelCatalogueEntry>, Vec<OpenCodeCatalogueOmission>), OpenCodeCatalogueError> {
...
pub fn compile_opencode_model_catalogue(
    ...
    for provider in ordered_providers {
        ...
        let (provider_entries, provider_omissions) =
            compile_provider_entries(health, context, &provider.id, &provider.models, policy)?;
...
mod tests {
    ...
    fn free_name_with_omitted_cost_remains_unknown() -> Result<(), Box<dyn std::error::Error>> {
        let named_free = model(
            "model-free",
            json!({"limit": {"context": 200_000, "output": 32_000}}),
        );
        let collection = compile_opencode_model_catalogue(
            &health(),
