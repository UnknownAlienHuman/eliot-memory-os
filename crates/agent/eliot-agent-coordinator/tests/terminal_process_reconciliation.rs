...
use eliot_agent_coordinator::{
    ...
    AttemptTerminalReconciliation, AttemptTelemetryInput, CoordinatedAttemptState,
...
#[test]
fn terminal_attempt_with_live_process_is_unreconciled() -> TestResult {
    let input = telemetry(
...
fn terminal_attempt_with_exited_process_reconciles_without_missing_process_alert() -> TestResult {
    ...
    assert!(!projection.alerts.contains(&AttemptAlertCode::ProcessMissing));
    Ok(())
}
