...
fn route(provider: &str, model: &str, suffix: &str) -> RouteFingerprint {
    let digest = |seed: &str| {
        serde_json::from_value::<LowercaseSha256>(serde_json::json!(sha256_hex(
            format!("swarm-fixture-{seed}-{suffix}").as_bytes()
        )))
...
fn selected(
    ...
    compile_model_selection(catalogue, preferences, ModelRole::Worker, selection_id, NOW)
}
...
fn missing_providers_are_plan_gaps_not_healthy_empty_state() -> TestResult {
    ...
    assert!(
        view.gaps
            .contains(&SwarmProjectionGap::ProviderUnavailable {
