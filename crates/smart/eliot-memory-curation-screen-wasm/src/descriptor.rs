//! Guest descriptor and pure-world import gate bound to the real typed world.
//!
...
//! sources by hand-copying. The typed `memory-curation-screen` world file is
//! present and readable on this base, but its owning issue #756 is OPEN (not
//! accepted), recorded explicitly as `PRESENT_NOT_ACCEPTED`
//! (`ContractChallenge` path) instead of an invented ABI.
...
/// WIT package identity from the readable `memory-curation-screen.wit`.
pub const WORLD_PACKAGE: &str = "eliot:current@0.1.0";
/// WIT world name from the readable `memory-curation-screen.wit`.
pub const WORLD_NAME: &str = "memory-curation-screen";
/// WIT export name bound by this guest (`screen: func(...)`).
pub const EXPORT_NAME: &str = "screen";
/// Screen subtype handled by this guest.
pub const HANDLER_SUBTYPE: &str = "screen";
...
pub const TYPED_WORLD_NAME: &str = "memory-curation-screen";
...
/// Accepted native contract name preserved by this guest (owner #586).
pub const NATIVE_CONTRACT: &str = "eliot.smart.memory-curation-screen";
/// Accepted native contract version preserved by this guest.
pub const NATIVE_CONTRACT_VERSION: &str = "1.0.0";
/// Owning issue of the native A-20 screen invoked exactly once per request.
pub const NATIVE_OWNER: &str = "#588";

/// Exact bytes of the owning WIT contract at compile time (read-only dep).
pub const GUEST_WIT_BYTES: &[u8] =
    include_bytes!("../../../../bins/eliot-wasm-host/wit/typed/memory-curation-screen.wit");
...
pub struct ComponentDescriptor {
    ...
    /// Screen subtype (must equal [`HANDLER_SUBTYPE`]).
    ...
    /// Accepted native contract name (must equal [`NATIVE_CONTRACT`]).
    pub native_contract: String,
    /// Accepted native contract version (must equal [`NATIVE_CONTRACT_VERSION`]).
    pub native_contract_version: String,
...
pub fn descriptor() -> ComponentDescriptor {
    ComponentDescriptor {
        ...
        native_contract: NATIVE_CONTRACT.to_owned(),
        native_contract_version: NATIVE_CONTRACT_VERSION.to_owned(),
