...
///
/// `expected_revision`, when present, is the source revision the retrieval
/// owner observed when compiling the plan; the admission freshness gate
/// compares it against each candidate's actual source revision through
/// `check_plan_revisions`. Absence means the owner states no revision
/// expectation for that source and no revision verdict applies to it.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize, JsonSchema)]
#[serde(deny_unknown_fields)]
pub struct SourceProjectionFence {
    ...
    /// Expected source revision for revision comparison, if the retrieval
    /// owner observed one at plan compilation time.
    ...
    pub fn validate(&self) -> Result<(), ContextError> {
        ...
        for entry in &self.source_projection_fences {
            ...
            if let Some(revision) = entry.expected_revision.as_deref() {
                validate_plan_text(
                    revision,
                    "retrieval_plan.source_projection_fences.expected_revision",
                )?;
            }
        }
