...
use eliot_dreamer_contracts::{
    ...
    ValidationPolicy, ValidationReceipt,
};
use eliot_dreamer_implementation_brief::{
    ArchitectureAlignment, CurrentEvidenceTarget, EvidenceVerdict,
    IMPLEMENTATION_BRIEF_SCHEMA_VERSION, ImplementationBriefInput, ImplementationDenominator,
    ImplementationEvidence, ImplementationMechanism, ImplementationObligation,
    ImplementationSourceSnapshot, ImplementationSourceStatus, ImplementationStatement,
    ImplementationStatementKind, ProofStage,
};
...
pub fn fixture() -> ImplementationBriefInput {
    ...
    let architecture_bytes =
        b"Kernel validates mechanically; Governor owns semantic admission.".to_vec();
    let implementation_bytes =
        b"Use one named transition validator under the current State Fence.".to_vec();
    ...
    let mut validation_policy = ValidationPolicy::new("validator-651", 1, 1_048_576);
