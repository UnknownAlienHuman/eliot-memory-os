...
use eliot_dreamer_implementation_brief::{
    ArchitectureAlignment, EvidenceVerdict, IMPLEMENTATION_BRIEF_PROOF_CEILING,
    ImplementationBriefDisposition, ImplementationBriefError, ImplementationBriefInput,
    ImplementationGapClass, ImplementationSourceStatus, MechanismDisposition, ProofStage,
    StageDisposition, project_implementation_brief,
};
...
fn first_stage(input: &ImplementationBriefInput) -> StageDisposition {
    project_implementation_brief(input)
        .expect("projection")
        .mechanisms[0]
        .obligations[0]
        .stages[0]
...
fn add_contradictory_evidence(input: &mut ImplementationBriefInput) {
    ...
    evidence.detail =
        "independent negative observation contradicts the favorable result".to_owned();
    evidence.evidence_digest.clear();
    input.evidence.push(evidence);
    input
        .denominator
...
fn multi_owner_multi_mechanism() {
    ...
    assert_eq!(
        output.mechanisms[0].owner.as_deref(),
        Some("governor-transition-owner")
    );
    ...
    assert!(
        output
            .mechanisms
            .iter()
            .all(|value| value.disposition == MechanismDisposition::Supported)
    );
}
...
fn source_lifecycle_is_preserved() {
    ...
    assert_eq!(
        accepted.implementation_source_status,
        Some(ImplementationSourceStatus::Accepted)
    );
    for status in [
        ...
        assert_eq!(
            output.disposition,
            ImplementationBriefDisposition::Unsupported
        );
    }
}
...
fn governing_architecture_binding_is_retained() {
    ...
    let output = project_implementation_brief(&input).expect("projection");
    assert_eq!(
        output.architecture_source_handle.as_deref(),
        Some("architecture-source")
    );
    assert_eq!(output.mechanisms[0].architecture_refs, vec!["arch-1"]);
    assert_eq!(
        output.implementation_source_handle.as_deref(),
...
fn constitutional_conflict_blocks_without_rewrite() {
    let mut input = fixture();
    input.implementation_source.as_mut().unwrap().statements[0].alignment =
        ArchitectureAlignment::Conflict;
    reseal(&mut input);
    let output = project_implementation_brief(&input).expect("projection");
    assert_eq!(output.disposition, ImplementationBriefDisposition::Blocked);
    assert_eq!(
        output.mechanisms[0].disposition,
        MechanismDisposition::Deviated
    );
    assert!(output.gaps.iter().any(|gap| gap.class
        == ImplementationGapClass::ArchitectureConflict
        && gap.owner == "architecture-precedence-owner"));
}
...
fn unspecified_mechanism_remains_unknown() {
    ...
    input
        .denominator
        .mechanism_ids
        .push("mechanism-unspecified".to_owned());
    input.denominator.complete = false;
    reseal(&mut input);
    let output = project_implementation_brief(&input).expect("projection");
    let unknown = output
...
fn support_axes_are_independent_not_a_total_ladder() {
    ...
    assert_eq!(
        evidence.implementation_support,
        ImplementationSupport::CurrentVerified
    );
    assert_eq!(
        evidence.evidence_execution_status,
        EvidenceExecutionStatus::Executed
    );
    assert_eq!(
        evidence.support_observation_state,
...
fn target_requirement_is_not_current_support() {
    ...
    assert_ne!(
        project_implementation_brief(&input).unwrap().disposition,
...
fn package_pass_does_not_imply_edge_or_runtime() {
    let mut input = fixture();
    input.obligations[0].required_stages =
        vec![ProofStage::Package, ProofStage::Edge, ProofStage::Runtime];
...
fn stale_and_incompatible_target_evidence_remain_visible() {
    ...
    assert_eq!(
        output.mechanisms[0].obligations[0].stages[0].disposition,
        StageDisposition::Conflicted
    );
}
...
fn contradictory_evidence_is_retained() {
    ...
    assert!(
        stage
            .evidence
            .iter()
            .any(|item| item.verdict == EvidenceVerdict::Passed)
    );
    assert!(
        stage
            .evidence
            .iter()
            .any(|item| item.verdict == EvidenceVerdict::Failed)
    );
}
...
fn partial_denominator_cannot_claim_all_current() {
    ...
    input
        .denominator
        .mechanism_ids
        .push("mechanism-missing".to_owned());
    reseal(&mut input);
    let output = project_implementation_brief(&input).expect("projection");
...
fn status_vector_and_missing_obligation_are_both_exposed() {
    ...
    assert!(
        output
            .gaps
            .iter()
            .any(|gap| gap.stage == Some(ProofStage::Edge))
    );
}
...
fn gap_handoff_names_owner_not_work_plan() {
    ...
    let gap = output
        .gaps
        .iter()
        .find(|gap| gap.stage == Some(ProofStage::Product))
        .unwrap();
...
fn bounded_output_retains_omissions() {
    ...
    assert!(
        output
            .omissions
            .iter()
            .any(|item| item.omission_id == "implementation-source-unavailable")
    );
}
...
fn privacy_authority_effect_and_proof_ceiling_are_bound() {
    ...
    assert_eq!(
        input.self_query.policy.privacy,
        PrivacyHandling::Unrestricted
    );
    assert_eq!(
        input.self_query.policy.authority_ceiling,
...
fn independent_bounds_fail_one_over() {
    ...
    reseal(&mut work);
    assert!(matches!(
        project_implementation_brief(&work),
        Err(ImplementationBriefError::Bound {
    ...
    reseal(&mut output);
    assert!(matches!(
        project_implementation_brief(&output),
        Err(ImplementationBriefError::Bound {
...
fn set_order_is_deterministic() {
    ...
    assert_eq!(
        project_implementation_brief(&left).unwrap(),
        project_implementation_brief(&right).unwrap()
    );
}
...
fn seven_dimensions_and_upstream_receipt_are_consumed_intrinsically() {
    ...
    assert_eq!(output.disposition, ImplementationBriefDisposition::Complete);
    assert_eq!(
        input.self_query.validated_candidate.validated.receipt,
        receipt_before
    );
}
...
fn malformed_and_oversized_input_fails_boundedly() {
    ...
    value
        .as_object_mut()
        .unwrap()
        .insert("unknown_control".to_owned(), serde_json::json!(true));
...
fn mechanism_binds_accepted_implementation_and_architecture() {
    ...
    assert_eq!(
        output.implementation_source_status,
        Some(ImplementationSourceStatus::Accepted)
    );
    assert_eq!(
        output.implementation_source_digest,
        input
            .implementation_source
            .as_ref()
            .map(|value| value.source_digest.clone())
    );
