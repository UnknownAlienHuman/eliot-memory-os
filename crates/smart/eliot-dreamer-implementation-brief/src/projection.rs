...
use crate::{
    ArchitectureAlignment, EvidenceAxisSnapshot, EvidenceVerdict,
    ...
    ImplementationBriefProjection, ImplementationEvidence, ImplementationGap,
    ImplementationGapClass, ImplementationMechanism, ImplementationObligation,
    ImplementationOmission, ImplementationSourceStatus, MechanismAssessment, MechanismDisposition,
    ObligationAssessment, ProofStage, StageAssessment, StageDisposition,
...
#[expect(
    clippy::too_many_lines,
    reason = "explicit per-statement Architecture alignment keeps gap ownership auditable"
)]
fn assess_mechanism(
    input: &ImplementationBriefInput,
    mechanism_id: &str,
...
#[expect(
    clippy::too_many_lines,
    reason = "explicit per-stage evidence reconciliation keeps status axes auditable"
)]
fn assess_obligation(
    ...
    for stage in &obligation.required_stages {
        ...
        let mut matching = input
            .evidence
            .iter()
            .filter(|item| item.obligation_id == obligation.obligation_id && item.stage == *stage)
            .collect::<Vec<_>>();
