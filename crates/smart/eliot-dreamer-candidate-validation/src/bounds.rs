//! Borrowed input bounds applied before contract validation or cloning.
...
pub(crate) const MAX_CANONICAL_BYTES: usize = 1_048_576;
...
fn add_bytes(
    total: &mut usize,
    value: &str,
) -> Result<(), crate::error::DreamDraftValidationError> {
    *total =
        total
            .checked_add(value.len())
            .ok_or(crate::error::DreamDraftValidationError::Bound {
                field: "validation.input_fields",
                maximum: MAX_CANONICAL_BYTES,
                actual: usize::MAX,
            })?;
    if *total > MAX_CANONICAL_BYTES {
        return Err(crate::error::DreamDraftValidationError::Bound {
            field: "validation.input_fields",
            maximum: MAX_CANONICAL_BYTES,
            actual: *total,
        });
...
/// Accounts every nested string and collection before any expensive A-03
/// validation, canonicalization, or owned result construction.
