...
reactivate historical function-per-crate prototypes because a planning document
named them.
...
1. `ContextRecipe.recipe_revision` must equal `ContextInput.task_revision`.
   ...
   task view.
2. One input read set contains each `ContextAtom.atom_id` at most once. Duplicate
   atoms are rejected before sorting, budgeting or rendering.
...
5. Every required role has one declared role budget. A zero role budget is
   valid: required/protected atoms remain available as exact handles rather than
   being silently discarded.
6. Whole-unit admission remains intact. Do not split one semantic atom merely to
   make it fit.
...
`D3B_REACTIVE_CONTEXT_PULSE_01` remain separate Edge/Product proofs. Report all
unexecuted boundaries exactly.
