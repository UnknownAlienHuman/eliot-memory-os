...
use crate::{
    BindingCandidateId, CanonicalCueIdentity, CueContractError, Digest, TargetHandle, bounds,
};
...
pub enum BindingDisposition {
    /// Caller-supplied disposition; A-10 does not prove admission or snapshot membership.
    Admitted,
...
impl CueBindingCandidate {
    ...
    pub fn validate(&self) -> Result<(), CueContractError> {
        ...
        bounds::text(self.target.as_str(), "binding.target")?;
