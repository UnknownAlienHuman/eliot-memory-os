//! Derived rival probe-input view consumed by discriminative-probe planning.
//!
//! The rich rival-structuring implementation may retain additional packing and
//! diagnostic state. This derived view carries only the immutable identities,
...
//! [`RivalProbeInputSetDisposition`]). The two shapes serve different consumers
//! and hash different preimages, so they cannot share one digest input:
//! merging them would alter a frozen canonical digest. Only the projection
//! revision is re-exported at the `rival::` level, keeping a single canonical
//! crate-level definition. This module is a derived consumer view: its digest
//! is its own first encoding and never the canonical model-set digest.
//! Long-term consolidation is an auditor decision; this module intentionally
//! stays separate and keeps its own preimage.
...
/// Encoding version for the derived rival probe-input view (not the canonical model-set wire revision).
pub const RIVAL_PROBE_INPUT_SET_SCHEMA_VERSION: u32 = 1;

/// Derived-view completeness. No variant is a truth or winner verdict.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "SCREAMING_SNAKE_CASE")]
pub enum RivalProbeInputSetDisposition {
    Complete,
    Partial,
    InsufficientRivals,
    Unprobeable,
...
impl RivalObjectiveOmission {
    pub fn validate(&self) -> Result<(), ContractViolation> {
        ...
        validation::text(&self.detail, "rival.probe_input_set.omission.detail")
    }
}

/// Named constructor input for [`RivalProbeInputSet`] (derived view; never the canonical model set).
#[derive(Clone, Debug)]
pub struct RivalProbeInputSetParams {
    ...
    pub disposition: RivalProbeInputSetDisposition,
}

/// Immutable, bounded derived rival probe-input view used by later contract-only consumers.
#[derive(Clone, Debug, Eq, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct RivalProbeInputSet {
...
/// Digest preimage for [`RivalProbeInputSet`], excluding the digest field.
///
...
/// and declaration order mirror the [`RivalProbeInputSet`] content fields in their
...
/// tuple, so no prior digest value exists: this struct establishes this derived
/// view's own first encoding (never the canonical model-set digest), and any
/// field addition, removal, reorder, or rename alters the digest.
#[derive(Serialize)]
struct RivalProbeInputSetDigestPreimage<'a> {
    schema_version: u32,
    set_id: &'a ArtifactId,
    task_id: &'a TaskId,
    scope: &'a String,
    ...
    disposition: RivalProbeInputSetDisposition,
}

impl RivalProbeInputSet {
    /// Constructs and seals one derived probe-input view.
    pub fn new(mut params: RivalProbeInputSetParams) -> Result<Self, ContractViolation> {
        params
            .objectives
        ...
        let denominator = params
            ...
            .ok_or(ContractViolation::OutOfBounds {
                field: "rival.probe_input_set.objective_denominator",
                min: 0,
        ...
        let mut value = Self {
            schema_version: RIVAL_PROBE_INPUT_SET_SCHEMA_VERSION,
    ...
    /// Validates intrinsic shape and derived-view digest.
    pub fn validate(&self) -> Result<(), ContractViolation> {
        validation::preflight(self)?;
        self.validate_shape()?;
        validation::digest(&self.digest, "rival.probe_input_set.digest")?;
        if self.digest != self.compute_digest()? {
            return binding(
                "rival.probe_input_set.digest",
                "digest does not match the rival probe-input view",
            );
        }
        Ok(())
    }

    /// Computes the derived-view digest excluding the digest field.
    ///
    /// The preimage is [`RivalProbeInputSetDigestPreimage`]: the same 19 content
    /// fields, in the same order, under their struct field names.
    pub fn compute_digest(&self) -> Result<String, ContractViolation> {
        validation::canonical_digest(&RivalProbeInputSetDigestPreimage {
    ...
    fn validate_shape(&self) -> Result<(), ContractViolation> {
        if self.schema_version != RIVAL_PROBE_INPUT_SET_SCHEMA_VERSION {
            return binding(
                "rival.probe_input_set.schema_version",
                "unsupported probe-input view schema version",
            );
        }
        validation::artifact(&self.set_id, "rival.probe_input_set.set_id")?;
        validation::text(self.task_id.as_str(), "rival.probe_input_set.task_id")?;
        validation::text(&self.scope, "rival.probe_input_set.scope")?;
        self.state_fence
            .validate()
            .map_err(|error| ContractViolation::BindingMismatch {
                field: "rival.probe_input_set.state_fence",
                reason: error.to_string(),
            })?;
        for (digest, field) in [
            (&self.bundle_digest, "rival.probe_input_set.bundle_digest"),
            (
            ...
            (
                ...
                "rival.probe_input_set.model_coverage_digest",
            ),
            (
                &self.source_coverage_digest,
                "rival.probe_input_set.source_coverage_digest",
            ),
        ] {
        ...
        validation::sequence(self.objectives.len(), "rival.probe_input_set.objectives")?;
        validation::sequence(self.omissions.len(), "rival.probe_input_set.omissions")?;
        ...
        if usize::try_from(self.objective_denominator).ok() != Some(actual_denominator) {
            return binding(
                "rival.probe_input_set.objective_denominator",
        ...
        for objective in &self.objectives {
            ...
            if previous_objective.is_some_and(|prior| prior >= &objective.objective_id) {
                return binding(
                    "rival.probe_input_set.objectives",
        ...
        for omission in &self.omissions {
            ...
            if previous_omission.is_some_and(|prior| prior >= omission) {
                return binding(
                    "rival.probe_input_set.omissions",
        ...
        match self.disposition {
            RivalProbeInputSetDisposition::Complete
                if self.objectives.is_empty() || !self.omissions.is_empty() =>
            {
                return binding(
                    "rival.probe_input_set.disposition",
                    "complete probe-input view requires objectives and no omissions",
                );
            }
            RivalProbeInputSetDisposition::InsufficientRivals if self.declared_model_count >= 2 => {
                return binding(
                    "rival.probe_input_set.disposition",
