...
mod authenticode;
mod observation_models;
mod package_manifest;
mod package_relative_path;

use authenticode::verify_authenticode_handle;
pub use authenticode::{
    AuthenticodeError, AuthenticodeEvidence, AuthenticodeVerdict, AuthenticodeVerifier,
    MAX_PE_HEADER_BYTES, PeCoffError, PeCoffEvidence, WindowsAuthenticodeVerifier, parse_pe_coff,
};
#[cfg(all(test, windows))]
use authenticode::{
    AuthenticodeFileObservation, compare_authenticode_observations, hash_file,
    observe_authenticode_handle,
};
#[cfg(test)]
use authenticode::{
    MAX_AUTHENTICODE_CERT_DER_BYTES, MAX_AUTHENTICODE_PROVIDER_CHAIN_ELEMENTS,
    ProviderCounterSignerState, classify_wintrust_status, provider_chain_is_bounded,
    provider_countersigner_state, provider_der_length,
};
pub use observation_models::{
    PackageSourceFileObservation, PackageSourceObservation, PackageStagingObservation,
    StagedDirectoryReceipt, StagedFileReceipt, StagingReceipt,
};
