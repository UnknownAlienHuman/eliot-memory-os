...
use contract_models::{
    AbsenceObservationError, InstallerRootRequest, digest, ensure_system_service_spec,
    map_absence_observation_error, primitive_request, snapshot_identity, windows_path_digest,
...
mod primitive_tests {
    ...
    fn explicit_restore_failure_returns_typed_stage_and_code_before_drop() {
        ...
        api.restore = Err(5);
        {
            ...
            assert!(
                !guard.armed,
                "explicit typed failure must still disarm before Drop"
            );
        }
