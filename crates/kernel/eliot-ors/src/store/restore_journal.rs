...
impl RedbRecoveryStore {
    ...
    pub fn bind_restore_journal_stream(
        ...
        let write = self.database.begin_write().map_err(storage)?;
        {
            ...
            match meta.get(key.as_str()).map_err(storage)? {
                Some(value) => {
                    let stored: crate::restore_journal::RestoreJournalStreamBinding =
    ...
    pub fn ensure_restore_journal_schema(&self) -> Result<u32, OrsError> {
        let write = self.database.begin_write().map_err(storage)?;
        {
            ...
            match existing {
                ...
                Some(_) => {
                    return Err(integrity("restore_journal_schema", "unknown restore journal schema identity"));
                }
                None => {
                    meta.insert(
                        RESTORE_JOURNAL_SCHEMA_ROW,
                        RESTORE_JOURNAL_SCHEMA_IDENTITY,
                    )
                    .map_err(storage)?;
                }
            }
    ...
    pub fn append_restore_journal_intent(
        ...
        let receipt = {
            ...
            for entry in intents.iter().map_err(storage)? {
                ...
                let (entry_stream, _) = split_key(key.value()).ok_or(integrity("restore_journal_entry", "malformed journal row key"))?;
                    ...
                    && stored.operation.body_digest == operation.body_digest
                {
                    if stored.payload_sha256 != payload_sha256 || stored.payload != payload {
                        return Err(integrity("restore_journal_entry", "changed payload under the same journal operation identity"));
                    }
                    replay = Some(stored);
                    break;
                }
                ...
                match &head {
                    Some((sequence, _)) if stored.sequence <= *sequence => {
                        return Err(integrity("restore_journal_entry", "journal sequence is not monotone"));
                    }
            ...
            if let Some(existing) = replay {
                ...
                match (&operation.expected_predecessor, &head) {
                    ...
                    _ => {
                        return Err(integrity("restore_journal_entry", "expected predecessor does not match the current journal head"));
                    }
                }
    ...
    pub fn append_restore_journal_result(
        ...
        let receipt = {
            ...
            if let Some(existing) = results.get(key.as_str()).map_err(storage)? {
                ...
                stored.validate()?;
                if stored != *result {
                    return Err(integrity("restore_journal_result", "conflicting result for the same journal phase"));
                }
                let digest = stored.digest()?;
                ...
                if !answered {
                    return Err(integrity("restore_journal_result", "result answers no committed journal intent"));
                }
                results
    ...
    pub fn load_restore_journal_stream(
        ...
        for row in rows {
            if deduped.last().map(|(sequence, _)| *sequence) == Some(row.0) {
                return Err(integrity("restore_journal_entry", "duplicate journal sequence"));
            }
            deduped.push(row);
        }
        ...
        for (sequence, stored) in &deduped {
            match (&stored.operation.expected_predecessor, &previous) {
                ...
                (Some(_), None) if pruned_before.map_or(false, |mark| mark + 1 == *sequence) => {}
                _ => {
                    return Err(integrity("restore_journal_entry", "journal predecessor linkage is broken"));
                }
            }
    ...
    /// Loads one result row by stream and phase, if present.    pub fn load_restore_journal_result(
        &self,
        stream: &str,
        phase_operation: &str,
...
fn validate_journal_text(value: &str, field: &'static str) -> Result<(), OrsError> {    if value.trim().is_empty()
