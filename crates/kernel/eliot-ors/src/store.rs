...
/// revision/incarnation, predecessor chain, and content-segment commitment,
...
/// chain, the content-segment commitment over the compacted pairs, the
/// acked frontier and boundary that certified it, the schema and retention
...
impl RedbRecoveryStore {
    ...
    /// owner binding, certifying frontiers). Same labeled-digest encoding
    /// as the owner namespace digest, so a changed range never reproduces a
    /// stored commitment.
    fn bridge_position_segment_commitment(segment: &BridgePositionSegment<'_>) -> String {
        let mut pairs_text = String::new();
        for (sequence, event_id) in segment.pairs {
            pairs_text.push_str(&sequence.to_string());
            pairs_text.push(':');
            pairs_text.push_str(event_id);
            pairs_text.push(',');
        }
        let text = format!(
            "{BRIDGE_POSITION_RANGE_COMMITMENT_DOMAIN}\x1fnamespace={}\x1fstart={}\x1fend={}\x1fpredecessor={}\x1frevision={}\x1fincarnation={}\x1facked={}\x1fcompacted={}\x1fpairs={pairs_text}",
            segment.namespace,
            segment.start,
            segment.end,
            segment.predecessor,
        ...
        crate::model::sha256_hex(text.as_bytes())
    }
    ...
    fn bridge_position_compact_plan_in(
        ...
        plan.stopped_transient = prefix_transient || records_transient || handoffs_transient;
    ...
    fn compact_bridge_positions_in(
        ...
        let plan =
            Self::bridge_position_compact_plan_in(write, &access, compacted, range_end, budget)?;
