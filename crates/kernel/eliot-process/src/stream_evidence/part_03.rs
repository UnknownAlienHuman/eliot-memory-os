...
/// Exact SHA-256/count identity for the physical transport prefix `[0, byte_length)`.
///
/// This identity is independent of the bounded inline preview and does not grant
/// persistence, parsing, evaluation, or storage authority.
#[derive(Clone, Debug, Eq, JsonSchema, PartialEq, Serialize)]
#[serde(deny_unknown_fields)]
pub struct ProcessStreamTransportPrefixIdentity {
    sha256: String,
    byte_length: u64,
}
...
#[derive(Deserialize)]
#[serde(deny_unknown_fields)]
struct ProcessStreamTransportPrefixIdentityWire {
    sha256: String,
    byte_length: u64,
}

impl ProcessStreamTransportPrefixIdentity {
    /// Creates an exact physical-prefix content identity.
    pub fn new(
    ...
    fn validate(&self) -> Result<(), ProcessStreamEvidenceError> {
        validate_digest("transport_prefix.sha256", &self.sha256)?;
        if self.byte_length == 0 && self.sha256 != sha256_hex(&[]) {
            return Err(ProcessStreamEvidenceError::Invariant {
                field: "transport_prefix.sha256",
                reason: "zero-byte transport prefix must use the empty SHA-256 identity",
            });
        }
        Ok(())
    }

    /// SHA-256 over every byte in the exact physical transport prefix.
    pub fn sha256(&self) -> &str {
        &self.sha256
    }

    /// Length of the exact physical transport prefix.
    pub const fn byte_length(&self) -> u64 {
        self.byte_length
    }
}

impl<'de> Deserialize<'de> for ProcessStreamTransportPrefixIdentity {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        let wire = ProcessStreamTransportPrefixIdentityWire::deserialize(deserializer)?;
        Self::new(wire.sha256, wire.byte_length).map_err(de::Error::custom)
    }
}
