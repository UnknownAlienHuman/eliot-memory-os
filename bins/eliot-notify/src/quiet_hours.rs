...
impl QuietHours {
    ...
    /// Returns true when a Unix-millisecond clock reading falls inside the
    /// window's UTC clock hour. The request's authenticated Kernel clock is
    /// used; the policy never reads an ambient wall clock or creates a new
    /// authority timestamp.
    #[must_use]
    pub const fn contains_unix_ms(self, unix_ms: u64) -> bool {
...
mod tests {
    ...
    fn quiet_hours_suppress_only_noncritical_popups() {
        ...
        let action_required_quiet =
            select_delivery(PopupSeverity::ActionRequired, false, false, true);
        assert!(!action_required_quiet.popup);
        assert!(action_required_quiet.create_canonical);
        assert!(action_required_quiet.board_visible);
