...
///
/// Pending the poller join (issue #1191, J3): same routing and body binding
/// as [`serve_skill_pair`], but intake resolves canonical procedure
/// acceptance over the authenticated Kernel route and drives install→receipt
/// only for owner-accepted material. The J3 integration renames this to
/// `serve_skill_pair` and rewires the poller to it, removing the sync
/// refusing entry. Async only for the acceptance read; the composition drive
/// itself stays synchronous with guards never crossing an await.
pub async fn serve_accepted_skill_pair(
    composition: &DaemonComposition,
...
///
/// Pending the poller join (issue #1191, J3): the wire intake entry decodes
/// once here, the presented package digest resolves against the canonical
/// committed lifecycle-policy rows, and the canonical verdict — never the
/// wire procedure stamp — decides the drive. Accepted intakes bind their
