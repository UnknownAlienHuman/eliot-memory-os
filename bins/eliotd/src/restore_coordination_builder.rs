...
//! producing a plan.
//!
//! The coordination parameter vocabulary is the Store catalogue's closed
//! vocabulary, single-sourced (see the canonical re-exports below): the
//! named operation variant stays Store-owned — the caller supplies it and
//! this builder requires its class to be `RecoverySchema` — while the six
//! parameter keys are the catalogue-owned constants, so builder, catalogue,
//! and bridge row agree by construction.
//!
...
//! The coordination parameter vocabulary below is the Store catalogue's
//! closed vocabulary, single-sourced through the canonical
//! `eliot_store_api` constants: the M2 integration activated them as
//! `RecordRestoreCoordination` catalogue parameters, so builder, catalogue,
//! and bridge row agree by construction instead of by parallel spelling.
...
use eliot_store_api::{
    ...
    COORDINATION_PARAM_DESTINATION, COORDINATION_PARAM_FENCE_DIGEST, COORDINATION_PARAM_OPERATION_ID,
    COORDINATION_PARAM_PAYLOAD_DIGEST, EffectClass, EventProjectionRelationIntents,
    NamedMutationOperation, NamedMutationRequest, OperationIdentity, OperationManifestDigest,
    OrderingScopeId, PreparedTransition, ScopeId, SecurityContext, TransitionClass,
    validate_coordination_mutation_params,
};
use serde_json::Value;

pub use eliot_store_api::{
    COORDINATION_PARAM_ADMISSION_DIGEST as COORD_PARAM_ADMISSION_DIGEST,
    COORDINATION_PARAM_DECISION_DIGEST as COORD_PARAM_DECISION_DIGEST,
    COORDINATION_PARAM_DESTINATION as COORD_PARAM_DESTINATION,
    COORDINATION_PARAM_FENCE_DIGEST as COORD_PARAM_FENCE_DIGEST,
    COORDINATION_PARAM_OPERATION_ID as COORD_PARAM_OPERATION_ID,
    COORDINATION_PARAM_PAYLOAD_DIGEST as COORD_PARAM_PAYLOAD_DIGEST,
};
