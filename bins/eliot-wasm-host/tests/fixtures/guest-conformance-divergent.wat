(component
  ...
  (core module $guest
    ...
    ;; Intentionally divergent twin of guest-conformance.wat: wrapping
    ;; SUBTRACTION instead of addition. Same ABI, same framing, different
    ;; artifact identity and different output bytes, so the differential
    ;; harness must reject conformance against the declared reference.
    (func $run (type $run)
      (local $i i32)
      (local $out i32)
      (local $seed_byte i32)
