...
//!
//! Required grant hook (Beauvoir lane — Kernel callers; Ramu handle
//! deliberately closed, 72fd retained): the legitimate grant arrives as an
//! authenticated Kernel-issued [`RuntimePorts`] bundle (Governor +
//! Authority + source + promotion + P-03 + verifier ports, engine slot
//! filled by this host's `with_wasmtime_engine`). Nearest in-tree anchors
//! for the handshake shape (different domain — Kernel service lifecycle,
//! NOT a ports grant):
//! `KernelComposition::execute_daemon_request`
//! (`bins/eliot-kernel/src/daemon_request_dispatch.rs`), the host
//! admission receipt family
//! (`crates/kernel/eliot-kernel-service/src/lifecycle.rs`,
//! `reconcile_host_request_admission` /
//! `reconcile_native_worker_claim_admission`), and — for the Beauvoir
//! redirect — the user-automation service dispatch surface
//! (`crates/kernel/eliot-kernel-service/src/user_automation.rs`,
//! `UserAutomationService::dispatch` / `reconcile`; execution entry
//! `user_automation_execution.rs::execute_occurrence`), steered from
//! `luna-1779-kernel-callers-20260921` (`codex/1779-kernel-callers`,
//! seed `f423`). No callable hook yielding a `RuntimePorts` grant exists
//! in-tree as of this write: `execute_daemon_request` serves async
//! session-bound lifecycle frames, the lifecycle reconciliation functions
//! prove receipt/envelope binding without issuing ports, and the automation
//! dispatchers serve occurrence execution behind their own runtime port —
//! none yields a wasm-host port bundle. Calling any of them as a grant
//! source would misappropriate another domain's authority, so this gate
//! keeps returning `NoAdmissionChannel` (→ `Unavailable`/`PlanGap`
//! downstream; `UnknownOutcome` preserved where the runtime already
//! reports it) until Beauvoir's lane delivers the real call. The required
//! call, when it lands, is exactly: `resolve_kernel_port_grant()` →
//! Beauvoir's `fn (authenticated_kernel_channel) -> Result<RuntimePorts,
//! GrantError>` with the error mapped to [`PortGrantError`]; nothing else
//! in this binary changes shape.
