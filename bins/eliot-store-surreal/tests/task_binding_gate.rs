...
use eliot_store_surreal::task_binding_gate::{
    GateDisposition, TASK_SCOPE_INCOMPATIBLE, TASK_SELECTION_REQUIRED, gate_apply, map_rejection,
};
use serde_json::json;
...
#[test]
fn production_mapping_carries_exact_stable_codes() {
    // The real canonical write path (apply_with_authority) maps through
    // map_rejection before any provider I/O: helper decisions and the
    // surfaced failure can never disagree on the code.
    let missing = gate_apply(&context(None), &control(None, Vec::new()))
        .expect_err("missing binding must reject");
    assert_eq!(
        map_rejection(&missing),
        eliot_store_api::StoreError::InvalidField {
            field: "task_binding",
            reason: TASK_SELECTION_REQUIRED,
        }
    );
    let crossed = gate_apply(
        &context(Some("task-b")),
        &control(Some("task-a"), binding_refs("task-a")),
    )
    .expect_err("cross-task write must reject");
    assert_eq!(
        map_rejection(&crossed),
        eliot_store_api::StoreError::InvalidField {
            field: "task_binding",
