...
//!
//! Ported from donor `origin/codex/961-owner-readback-20260923@53801ae0`
//! (`bins/eliot-host/src/introduction_readback.rs`; no authorship change, no
//! duplicate owner). Unchanged: the donor body already resolved against
//! current main's front-door helpers and the `eliot_kernel_service` re-exports
//! ported in this branch.

use super::{
    HostComposition, HostError, connect_authenticated_kernel_front_door, kernel_control_request,
...
pub(crate) fn read_live_introductions(
    ...
    let expected_kernel_image =
        host.jobs.kernel_executable.as_ref().ok_or_else(|| {
            HostError::ProcessContour("approved Kernel image is missing".to_owned())
        })?;
    let query = IntroductionReadbackQuery { max_rows };
    query
        .validate()
