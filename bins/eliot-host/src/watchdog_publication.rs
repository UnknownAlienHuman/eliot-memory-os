use super::{
    ...
    OperationIdentity, OwnedDirectoryPublication, OwnedDirectoryRetirementOutcome,
    OwnedDirectoryRetirementPrecondition, Path, PathBuf, PlatformHandle, ProtectedRuntimePathLease,
    PublishedSupervisionIdentity, Read, SUPERVISION_LEASE_FILE_NAME, Seek, SignedSupervisionLease,
    SupervisionLeaseVerifier, SystemTime, UNIX_EPOCH, WATCHDOG_ADMISSION_FILE_NAME,
...
#[cfg(windows)]
pub(super) struct HostWatchdogPublicationObservation {
    path: PathBuf,
...
#[cfg(windows)]
pub(super) fn observe_host_watchdog_publication(
    path: &Path,
) -> Result<HostWatchdogPublicationObservation, HostError> {
...
#[cfg(windows)]
pub(super) fn verify_exact_current_watchdog_publication(
