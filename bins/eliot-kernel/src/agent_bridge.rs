...
use super::{
    ...
    AgentActivationResolutionDisposition, AgentActivationResolutionResult, AgentBridgeHandshake,
    AgentBridgeProfile, KernelComposition, activation_deadline_expired,
...
impl KernelComposition {
    ...
    fn revoke_all_agent_bridges(&self) -> Result<(), TransportError> {
        ...
        if let Ok(mut results) = self.agent_activation_results.lock() {
            results.clear();
        } else {
            return Err(TransportError::SessionFenced);
        }
    ...
    pub(super) fn submit_agent_activation_resolution_result(
        ...
        let mut results = self
            ...
            .lock()
            .map_err(|_| TransportError::SessionFenced)?;
        match Self::classify_activation_result_for_entry(results.get(&result.ticket_id), &result) {
            ActivationDecisionDisposition::ExactReplay => return Ok(()),
            ActivationDecisionDisposition::Conflict => {
                return Err(TransportError::IdentityConflict);
            }
        ...
        results.insert(ticket_id.clone(), result);
        pending.fifo.retain(|queued_id| queued_id != &ticket_id);
        drop(results);
        drop(pending);
        self.agent_activation_changed.notify_waiters();
    ...
    fn activation_response_frame_for_resolution(
        ...
        let authenticated = AgentBridgeAuthenticatedBinding {
            ...
            activation_generation: result.ticket_state_fence.resource_generation,
            state_fence: AgentBridgeActivationFence {
                authority_epoch: result.ticket_state_fence.authority_epoch,
                generation: result.ticket_state_fence.resource_generation,
                nonce: session_nonce,
            },
        ...
        loop {
            let outcome = {
                ...
                let results = self
                    ...
                    .lock()
                    .map_err(|_| TransportError::SessionFenced)?;
                pending.entries.get(&ticket.ticket_id).map(|entry| {
                    (
                        entry.decision.clone(),
                        results.get(&ticket.ticket_id).cloned(),
                    )
                })
            };
            if let Some((decision, result)) = outcome {
