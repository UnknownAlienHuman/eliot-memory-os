...
mod tests {
    ...
    fn issue_then_validate_round_trips() {
        let grant =
            issue_wasm_port_grant(&sealed_request(), &observed(), &host_facts()).expect("issue");
    ...
    fn handler_denies_without_live_admission() {
        ...
        stale_envelope.envelope_sha256 = stale_envelope.compute_digest().expect("reseal envelope");
        let stale_receipt = eliot_protocol::HostRequestAdmissionReceipt::issue(&stale_envelope)
