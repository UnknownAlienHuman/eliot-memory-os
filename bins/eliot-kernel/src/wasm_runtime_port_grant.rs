...
//! Kernel-side fact (`observed` below — fence, epoch, connection, clock)
//! is THREADED from Kernel state by the composing caller; this module
//! echoes it into the grant and checks coherence, never inventing it.
//! Registration of this issue/validate pair into a dispatch route and any
//! composition wiring belongs to Beauvoir's shared-Kernel
//! registration/composition span (see
...
/// facts. Denies expired requests, digest breaks, and any disagreement
/// between the request and Kernel observation (connection, fence, epoch
/// lineage). Issues no permits, no Governor observations, no keys: the
/// grant attests transport + freshness and carries caller-asserted
/// digests for downstream re-hashing.
...
mod tests {
    ...
    fn issue_then_validate_round_trips() {
        let grant = issue_wasm_port_grant(&sealed_request(), &observed()).expect("issue");
        ...
        let again = issue_wasm_port_grant(&sealed_request(), &observed()).expect("issue");
        assert_eq!(grant.grant_sha256, again.grant_sha256);
    }

    #[test]
    fn tampered_grant_field_fails_validation() {
        let mut grant = issue_wasm_port_grant(&sealed_request(), &observed()).expect("issue");
    ...
    fn expired_and_foreign_observations_fail_closed() {
        ...
        assert_eq!(
            issue_wasm_port_grant(&sealed_request(), &foreign_fence),
            Err(WasmGrantError::ObservationMismatch {
                field: "state_fence"
            })
        );
