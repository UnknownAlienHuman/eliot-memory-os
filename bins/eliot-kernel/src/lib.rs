...
pub use backup_restore::{
    KernelBackupRestore, KernelRestoreOutcome, phase_owner,
};
pub use backup_restore_ports::{
    KernelIsolatedDestination, KernelRestoreError, KernelRestoreJournal,
    RESTORE_ISOLATED_AREA, RESTORE_JOURNAL_DIR_NAME, RESTORE_JOURNAL_FILE_NAME,
    RESTORE_JOURNAL_IDENTITY, RESTORE_JOURNAL_OWNER_ID,
};
...
pub struct KernelComposition {
    ...
    /// restore journal cannot be opened below the work root; restore effects
    /// are refused in that state and unrelated Kernel work is unaffected.
    /// Caller integration that drives restores arrives separately (#962).
    #[allow(
        dead_code,
