...
5. the builder verifies R(C) before omitting any artifact
6. stage a bundle carrying the approval reference and digests
7. the Authenticode finalizer independently re-verifies R(C), signs the
   allowed roles, and carries the same approval identity
8. release/install readback verifies the same approval and exact candidate
```

No step amends `C` after approval. Any candidate or denominator movement requires a new approval operation. There is no commit-fixed-point search, because the approval's canonical preimage digest covers the candidate commit **and** the candidate tree object, so the approval is never part of the tree it certifies.

**The explicit input.** The approval reaches the build through exactly one parameter:

```text
-GovernorRetirementApproval <absolute path to the detached approval artifact>
```

There is no environment-variable selection, no default path inside the repository, and no directory search. The artifact is pinned and read through the release safe path/handle rules and is always outside the candidate tree. The finalizer takes the same input as `-RetirementApproval`. An unset input is *absence*, never implicit retirement: absence means `retained-legacy-entrypoint` while the legacy source is still valid, and `broken/blocked` when the package or target is already absent.

**The trust root cannot be supplied by the caller.** The only thing that can admit a `retirement-approval` issuer is the root-owned tracked trust policy `scripts/lib/governor-retirement-approval-trust.json`, read at the pinned source commit through the same pinned path rules as every other release input. The caller supplies the approval artifact only, so the same caller can never present an arbitrary approval together with an arbitrary trust root. The Authenticode Code Signing EKU is **not** semantic retirement authority: the finalizer's existing certificate machinery is reusable only as a cryptographic primitive, and a trust-policy claim that names an Authenticode code-signing thumbprint for the `retirement-approval` role is refused outright.

**Current issuer state is `ISSUER_UNAVAILABLE` (fail closed).** Current `main` has no production release-retirement approval issuer, and the retirement role is a semantic owner decision this issue is not authorized to invent. `admitted_issuers` in the trust policy is therefore empty, so no detached approval can be actionable today and every release of a candidate that still contains the legacy facade stays `retained-legacy-entrypoint`. That is the correct state, not a defect: a well-formed approval JSON file, a nonblank `approval_identity`, or a candidate-written receipt can never authorize retirement. The root owner admits exactly one issuer identity for the `retirement-approval` role in that trust policy; before then, retirement is unreachable through the intended path.

**Independent denominator.** The candidate's `CONSUMER_SURFACES` table stays useful reviewed migration evidence, but it cannot certify its own completeness. The closure is recomputed by `Get-GovernorRetirementConsumerClosure` in `scripts/lib/governor-retirement-approval.ps1` — outside the retiring facade package, so a candidate cannot shrink the table and the detector at the same time. It enumerates every tracked file of `C`, classifies every file naming the retiring surface, and requires all five path families (`source_and_automation`, `retiring_plugin_tree`, `host_integration_manifest`, `desktop_surface`, `documentation_and_workstream`) to be present. Any unclassified tracked reference, or any missing family, makes the closure `INCOMPLETE` and blocks approval rather than disappearing from the denominator.

**Evidence carried by one approval identity.** When, and only when, a detached approval is admitted, one identity is bound to one exact candidate and carried by the plan, `STAGED_PAYLOAD_MANIFEST.json`, `RELEASE.json`, `SHA256SUMS.json`, `SIGNING_VERIFIED.json` and every readback. The plan additionally reports `governor_retirement_approval` (how the input arrived) and `governor_retirement_approval_trust` (the pinned trust blob, release-policy revision and issuer state). Any disagreement between these about which approval authorized the omission fails the bundle closed.

**Offline finalization.** A retired bundle carries the immutable approval and the exact trust policy beside the payload as `GOVERNOR_RETIREMENT_APPROVAL.json` and `GOVERNOR_RETIREMENT_APPROVAL_TRUST.json`; their digests are bound by `SHA256SUMS.json`. The finalizer re-reads both through the release safe path/handle rules and re-derives the approval body from those exact bytes, so finalization and readback need no network. Missing or stale trust material refuses retirement rather than degrading to shape-only. Because the certificate roles are selected by what is present, signing the remaining files of an unapproved omitted set can never convert it into an approved one: the finalizer re-resolves `R(C)` and fails closed before any signature is applied.

**Exact replay.** The canonical request hash covers the requested action — candidate commit and tree, approved denominator, consumer dispositions, replacement/removal decisions, legacy target identity, policy revisions, and rollback/reopen conditions — and excludes the owner evidence (issuer receipt, issuance instant, approver principal, file content digest). The same approval operation over the same candidate therefore re-derives byte-identical operation and request identities and returns the same decision, while any changed candidate, denominator or disposition set under the same operation produces a different request hash and **conflicts** instead of replaying.

**Source absence stays fail closed.** A checkout whose package or target is missing, but which has no valid detached approval, is `broken/blocked` — never retired. No approval can certify a source object that no longer exists.

**Proof ceiling.** The approval authorizes **omission from this exact release candidate** and nothing more. Its recorded proof ceiling is `DETACHED_RELEASE_CANDIDATE_OMISSION_ONLY (no installed migration, no live Product behavior, no safe data deletion evidence)`. It does **not** prove that any installed instance was migrated, that live Product behavior is correct, or that deleting legacy data is safe; those remain #11 and #18 evidence. The per-consumer replacement owner or explicit product-removal decision is preserved as declared evidence in `RELEASE.json` and the plan, and staging the legacy Claude selection when the disposition is retired throws directing to `-ClaudeCodeFrontDoor agent-bridge`.

**Simulation stays simulation.** `-PlanRetiredGovernor` remains a `-PlanOnly`-only proof sketch. It renders the retired layout for inspection, is visibly `SIMULATED_NOT_ADMITTED`, writes no bundle, carries no approval identity, and is never migrated into accepted evidence by a later stage, finalizer or readback. It can neither issue nor substitute the approval input: supplying `-GovernorRetirementApproval` together with `-PlanRetiredGovernor` is refused outright. Any old or self-declared retired plan or bundle stays `SIMULATED_NOT_ADMITTED` and is never grandfathered.

**Retained builds are unchanged.** When no approved retirement is supplied and the legacy source is still valid, today's retained slice stages byte-identically and the new fields simply report the absence explicitly. The retirement decision logic itself lives in `scripts/lib/governor-retirement-approval.ps1`, deliberately outside `crates/eliot-app`, so retiring the facade package can never remove or edit the contract that governs its own retirement. Work option 1 — retire the governor artifact — is implemented conditionally. While `crates/eliot-app` is a workspace member the disposition is `retained-legacy-entrypoint` and nothing changes; retirement is owner-proven (issues #2892/#2968), never presence-driven: the typed resolver returns `Retained`, `RetirementCandidate`, `Retired`, or `MalformedOrAmbiguous`, and only a detached owner approval `R(C)` that verifies for the exact source commit and tree, under an owner-admitted issuer, against the independently recomputed consumer closure, selects `Retired` (no governor build, no `governor-gated-legacy` include, and the Codex marketplace/plugin subtree leaves the release together with the binary). Missing/duplicated package, missing target, metadata failure, unexpected shape, and source absence without a verified approval abort explicitly; no ambient switch exists. An approval-admitted retired plan carries an explicit `retired` `governor_disposition` bound to the approval content digest (the plan, `STAGED_PAYLOAD_MANIFEST.json`, `RELEASE.json`, and `SHA256SUMS.json` carry the same approval identity, and the verifier re-resolves the detached approval from the pinned commit instead of trusting the builder string), omits the legacy Claude include (emitting `claude-frontdoor-retired`), records the approval-admitted per-consumer replacement or explicit product-removal decision (no `eliot setup` substitution is claimed without owning Product contract proof; a missing replacement stays blocked/partial), and staging the legacy Claude selection throws directing to `-ClaudeCodeFrontDoor agent-bridge`, so plan and stage agree. Installer and runtime role requirements stay separate: what the installer's `BUNDLE_BINARIES`/`REQUIRED_ROLES` need does not decide what the shipped host integrations need, so retirement requires the complete consumer denominator with a per-consumer admitted replacement or explicit removal decision — an installer-only argument never retires a runtime host surface. Re-home (Work option 2) is not implemented here and is refused: the `codex_controller` profile is implemented only under the legacy `crates/eliot-app/src/mcp_stdio.rs` (with its submodules) — zero implementers exist in `bins/`, in `crates/surfaces/eliot-mcp`, or anywhere else — so no current-owner entry point exists, and the behavior home is the `eliot-mcp` track per canon. Per I19.1 (no deletion before data/behavior owner and replacement proof) nothing is deleted in this slice; the legacy deletion itself stays owned by #18. Proof sketch without deletion: `-PlanRetiredGovernor` with `-PlanOnly` renders the retired layout for inspection only; it is visibly `SIMULATED_NOT_ADMITTED`, writes no bundle, alters no release artifacts/receipts/verification inputs, is never selected from ambient environment, and cannot stage, verify, sign, or publish. Already-generated self-declared retired plans/bundles are simulated/unadmitted, must be rebuilt from a detached owner approval, and are never grandfathered.
