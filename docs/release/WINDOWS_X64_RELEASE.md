...
The tracked and released `plugin.json` is cache-neutral: its version is the base SemVer, currently `0.1.0`, with no `+codex` build metadata. `PlanOnly` reports this as `codex_plugin_base_version`, and `RELEASE.json` records the same value. The installer must not rewrite either source or release payload. It materializes only its ELIOT-owned personal-plugin copy as `<base-version>+codex.<deterministic-content-token>` before invoking the Codex plugin lifecycle. The token is stable for the normalized Codex cache contract and changes when MCP, hooks, skills, or plugin metadata change. The cached binary is deliberately excluded because Codex executes the separately SHA-256-verified Governor at the installed source path; a binary-only Governor update therefore does not churn the active Codex cache. A timestamp-only token is reserved for manual local-development iteration.
