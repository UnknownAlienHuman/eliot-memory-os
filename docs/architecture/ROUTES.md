...
| `governor-skill-drive` | Daemon-side Skill candidate materialization drive: canonical acceptance lookup, owner-bound procedure rehydration, and Hotset install-to-receipt composition (issue #1191). | `bins/eliotd/src/skill_dispatch.rs`<br>`bins/eliotd/src/skill_acceptance_read.rs`<br>`bins/eliotd/src/skill_lifecycle_adapters.rs` | `skill validation and promotion`<br>`skill lifecycle`<br>`candidate materialization`<br>`canonical acceptance`<br>`hotset delivery` | `I7.13`<br>`I7.25` | `I7.12` |
