...
| `repository-root` | Root-level repository identity, licensing, tool pins, and contributor entry surfaces. | `.gitattributes`<br>`.gitignore`<br>`LICENSE`<br>`README.md`<br>`START.md`<br>`WORKFLOW.md`<br>`.typos.toml`<br>`Cargo.toml`<br>`Cargo.lock`<br>`Justfile`<br>`deny.toml`<br>`mcpls.toml`<br>`rust-toolchain.toml`<br>`tool-versions.json` | `repository root`<br>`license`<br>`toolchain pin` | `A0.4`<br>`A14.8`<br>`I0.14`<br>`I2.17`<br>`I18` | `I20` |
