...
4. **No global single-writer bottleneck.** Governor is the only application write authority, but internally uses ordered writer lanes partitioned by project. Writes inside one project remain ordered; independent projects commit concurrently.
...
|---|---|---|
| entity-serialized ownership | project writer lanes with per-project FIFO heads | preserves causal order without a global writer bottleneck |
| transactional outbox | domain mutation, receipt, revision and outbox row in one DB transaction | notifications cannot claim a write that did not commit |
...
| event journal plus projections | immutable `memory_event` with materialized current state | audit/replay without forcing slow event-sourced reads |
...
```text
1. SurrealDB service
   C:\Tools\SurrealDB\surreal.exe start ... rocksdb://C:/ProgramData/Eliot/data/surrealdb
...
## 6.3. Canonical write model: event plus projections

Every admitted mutation produces:

```text
1. one or more immutable `memory_event` records;
2. updates to materialized current-state records;
3. a `write_receipt`;
4. scope revision increments;
5. optional outbox notification rows.
```

All five happen in one SurrealDB transaction.

This is not full event-sourcing dogma: reads use materialized tables for speed. The event journal exists for audit, replay and projection repair.
...
`ControlWalActor` performs bounded group commits:
...
## 8.6. Project writer lanes

Governor is the sole application writer, but it is not a single global serialized loop.
...
7. Append immutable `memory_event` rows.
8. Apply materialized projection changes.
9. Create typed relation edges.
10. Increment relevant ScopeState revisions and set `last_project_sequence`.
11. Append tamper-evident project event-chain fields:
      prev_event_hash, event_hash.
12. Write canonical `write_receipt`.
13. Write outbox rows for subscribers/report jobs.
14. Commit transaction.
15. Update in-memory ScopeHeadCache.
16. Mirror receipt and remove pending record from redb.
17. Notify MCP waiters/resources subscribers.
```

A single envelope is the atomic unit. V1 does not combine unrelated envelopes into one DB transaction. Parallel lanes provide throughput without coupling failure domains.
...
## 12.10. Parallel writes through Governor

Many agents can submit simultaneously because:

```text
MCP/IPC admission runs concurrently;
redb staging gives durable backpressure;
project sequence provides order;
writer lanes isolate projects;
...
Lifecycle:
  ...
  any -> quarantined
  forgotten -> restored
  any -> hard_deleted through tombstone path
```
...
The data is valuable and must be migrated, not discarded.
...
4. Install/configure SurrealDB Windows service in server mode with a new local non-OneDrive RocksDB data root.
...
binary = "C:/Tools/SurrealDB/surreal.exe"
storage_engine = "rocksdb"
storage_path = "C:/ProgramData/Eliot/data/surrealdb"
bind = "127.0.0.1:8123"
legacy_surreal_kv_path = "C:/Users/kleym/OneDrive/Documents/MCP/.eliot/surrealdb"
...
| Write concurrency | ordered project writer lanes |
...
Parallelism is controlled by leases, revisions and project writer lanes.
