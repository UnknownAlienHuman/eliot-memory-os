...
| P4 | `RATIFIED_PROVIDER_FIRST` | `ARCH-DEV-01` is applied as provider-first ordering: the real Kernel agent-bridge provider precedes broad hardening. W0/W3 artifacts are not called a vertical spine, and no new split is authorized. The admission and semantic-resolution ownership boundary is frozen before product mutation. | The current `KernelAdmissionRequired` refusal is static evidence of the missing provider, not a runtime spine. A production-reachable typed denial can prove only the R13 step-1 route; only an exact resolver-bound response can close step 2. | Revisit after admitted `eliot.agent-bridge.activate` plus an end-to-end loop; until then the provider-first order remains mandatory. | `swarm/decisions/R13-PROVIDER-1-ADMISSION-AND-RESOLUTION.md`; `docs/architecture/ELIOT_ARCHITECTURE.md:277-308`; `bins/eliot-agent-bridge/src/lib.rs`; v1.5 §2, §8, Appendix P4. |
...
- Provider #1 is internally ordered by
  `R13-PROVIDER-1-ADMISSION-AND-RESOLUTION.md`; a handler-only or
  Kernel-synthesized semantic response is not an implementation of P4.
