...
| P6 | `EXECUTED_IN_CURRENT_OVERLAY` | Workspace count drift is corrected to 126 in CI and `PROJECT_MAP`; the correction is preserved in the next commit and does not open a new WorkItem. | Static topology alignment only; hosted execution is not claimed. | Any later workspace topology change must update both carriers in the same change; mismatch marks the map stale and blocks that topology gate. | `.github/workflows/ci.yml`; `docs/PROJECT_MAP.md`; v1.5 §6 and Appendix P6. |
...
- P6 is not durable until the current `PROJECT_MAP` correction is committed.
