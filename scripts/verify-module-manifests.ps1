...
function Get-ExpectedDocument([string]$Root,$Meta,[string[]]$Union){$doc=$Meta.Document;$ids=[Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal);foreach($id in @($doc.workspace_members)){[void]$ids.Add([string]$id)};$packages=@($doc.packages|Where-Object{$ids.Contains([string]$_.id)}|Sort-Object name -Culture '');if($packages.Count-ne 125){Fail "Cargo workspace package count is $($packages.Count)"};$byPath=@{};foreach($p in $packages){$byPath[((Resolve-Path $p.manifest_path).Path).ToLowerInvariant()]=$p.name};$edges=[Collections.Generic.List[object]]::new();foreach($p in $packages){foreach($d in @($p.dependencies)){$path=Get-JsonProp $d 'path';if($null-eq $path){continue};$key=((Resolve-Path (Join-Path $path 'Cargo.toml')).Path).ToLowerInvariant();if(-not $byPath.ContainsKey($key)){Fail "unresolved workspace dependency: $($p.name)"};$edges.Add([pscustomobject]@{from=$p.name;to=$byPath[$key];kind=if($null-eq $d.kind){'normal'}else{[string]$d.kind};target=(Get-JsonProp $d 'target')})}}
...
function Assert-GeneratorCliRejected([string]$Root,[string[]]$Arguments,[string]$ExpectedPattern) {
    ...
    if(($stdout+"`n"+$stderr)-notmatch $ExpectedPattern){Fail "generator CLI negative fixture returned wrong refusal: $stderr"}
}
