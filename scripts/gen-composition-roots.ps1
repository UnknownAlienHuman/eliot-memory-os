...
function Invoke-GeneratorSelfTest {
    $predicate = Get-PredicateDefinition 'eliot-r13-two-token-harness' 'useful_work'
    if ($predicate.kind -cne 'reachable_effect' -or @($predicate.required_source_fragments) -cne @('eliot_kernel::r13_os_harness::run()')) {
        Fail 'R13 harness predicate self-test mismatch'
    }
    $mismatchFailed = $false
    try { Get-PredicateDefinition 'eliot-r13-two-token-harness' 'typed_refusal' | Out-Null } catch { $mismatchFailed = $true }
    if (-not $mismatchFailed) { Fail 'classification/predicate mismatch self-test did not fail' }
    $selfTestRoot = Join-Path ([System.IO.Path]::GetTempPath()) ('composition-roots-generator-selftest-' + [guid]::NewGuid().ToString('N'))
    New-Item -ItemType Directory -Path $selfTestRoot -Force | Out-Null
    try {
        $outA = Join-Path $selfTestRoot 'inventory-a.json'; $resultA = Join-Path $selfTestRoot 'result-a.json'
        $outB = Join-Path $selfTestRoot 'inventory-b.json'; $resultB = Join-Path $selfTestRoot 'result-b.json'
        & pwsh -NoProfile -File $PSCommandPath -OutputPath $outA -ResultPath $resultA | Out-Null
        if ($LASTEXITCODE -ne 0) { Fail 'determinism self-test first generation failed' }
        & pwsh -NoProfile -File $PSCommandPath -OutputPath $outB -ResultPath $resultB | Out-Null
        if ($LASTEXITCODE -ne 0) { Fail 'determinism self-test second generation failed' }
        if ((Read-Utf8 $outA) -cne (Read-Utf8 $outB) -or (Read-Utf8 $resultA) -cne (Read-Utf8 $resultB)) { Fail 'generator output is not byte deterministic' }
    } finally {
        if (Test-Path -LiteralPath $selfTestRoot) { Remove-Item -LiteralPath $selfTestRoot -Recurse -Force }
    }
    Write-Output 'COMPOSITION_ROOTS_GENERATE_SELF_TEST: PASS predicate=1 mismatch=1 deterministic=1'
}
...
$catalogueJson = @'
[
  {"target":"eliot","classification":"useful_work","operational_scope":"product_cli","main":{"path":"bins/eliot/src/main.rs","line":332,"symbol":"main","text":"fn main() -> Result<()> {"},"call_chain":[{"path":"bins/eliot/src/main.rs","line":346,"symbol":"run","text":"fn run() -> Result<i32> {"},{"path":"bins/eliot/src/main.rs","line":349,"symbol":"run","text":"    match cli.command {"}],"gate_condition":"valid CLI command and its command-specific admission/configuration gates","effect_boundary":"CommandPort and governed command handlers; effects remain behind the selected command port","falsifier":"A current main-to-run path reaches only typed refusal or no command handler for every command"},
  {"target":"eliot-agent-bridge","classification":"useful_work","operational_scope":"product_ingress","main":{"path":"bins/eliot-agent-bridge/src/main.rs","line":41,"symbol":"main","text":"fn main() {"},"call_chain":[{"path":"bins/eliot-agent-bridge/src/main.rs","line":65,"symbol":"kernel_ports_with_declaration","text":"        match kernel_ports_with_declaration(&config.client_declaration) {"},{"path":"bins/eliot-agent-bridge/src/lib.rs","line":623,"symbol":"kernel_ports_with_declaration","text":"pub fn kernel_ports_with_declaration("},{"path":"bins/eliot-agent-bridge/src/main.rs","line":72,"symbol":"BridgeRunner::new","text":"    let mut runner = match BridgeRunner::new("},{"path":"bins/eliot-agent-bridge/src/main.rs","line":85,"symbol":"main","text":"    for line in io::stdin().lock().lines() {"}],"gate_condition":"valid profile/client declaration and authenticated Kernel front-door admission","effect_boundary":"BridgeRunner attach/forward processing over stdin using the admitted Kernel ports","falsifier":"a valid declaration and admitted ports cannot reach BridgeRunner construction or the stdin request loop"},
  ...
  {"target":"eliot-governor","classification":"useful_work","operational_scope":"product_governor","main":{"path":"crates/eliot-app/src/main.rs","line":2027,"symbol":"main","text":"fn main() -> Result<()> {"},"call_chain":[{"path":"crates/eliot-app/src/main.rs","line":2036,"symbol":"main","text":"            let cli = Cli::parse();"},{"path":"crates/eliot-app/src/main.rs","line":2047,"symbol":"dispatch_command","text":"            runtime.block_on(Box::pin(dispatch_command("}],"gate_condition":"valid CLI command plus configured runtime and command-specific authority gates","effect_boundary":"Governor command handlers and their governed app/store ports","falsifier":"the spawned main thread cannot reach dispatch_command for any command"},
  ...
  {"target":"eliot-kernel","classification":"useful_work","operational_scope":"product_kernel","main":{"path":"bins/eliot-kernel/src/main.rs","line":190,"symbol":"main","text":"async fn main() {"},"call_chain":[{"path":"bins/eliot-kernel/src/main.rs","line":191,"symbol":"parse_launch_options","text":"    let options = match parse_launch_options(std::env::args_os().skip(1)) {"},{"path":"bins/eliot-kernel/src/main.rs","line":213,"symbol":"KernelConfig::new","text":"        KernelConfig::new(options.work_root.clone()).require_descriptor_supervision_authority();"}],"gate_condition":"validated launch, host binding, store bootstrap, and eliotd descriptor","effect_boundary":"authenticated Kernel control pipe and admitted runtime services","falsifier":"all validated launch inputs still end before Kernel listener/service composition"},
  ...
  {"target":"eliot-r13-two-token-harness","classification":"useful_work","operational_scope":"test_only","main":{"path":"bins/eliot-kernel/src/r13_two_token_harness.rs","line":3,"symbol":"main","text":"fn main() {"},"call_chain":[{"path":"bins/eliot-kernel/src/r13_two_token_harness.rs","line":4,"symbol":"eliot_kernel::r13_os_harness::run","text":"    std::process::exit(eliot_kernel::r13_os_harness::run());"}],"gate_condition":"r13-os-harness feature enabled and explicit approved harness arguments supplied; off-feature or incomplete arguments refuse before mutation","effect_boundary":"bounded Windows SCM/service and named-pipe harness evidence under the pre-provisioned harness root; no normal Kernel startup path","falsifier":"the bin is runnable without r13-os-harness and explicit harness arguments, or its main reaches any effect without the exact r13_os_harness::run() call"},
  ...
  {"target":"eliotd","classification":"useful_work","operational_scope":"product_daemon","main":{"path":"bins/eliotd/src/main.rs","line":50,"symbol":"main","text":"fn main() {"},"call_chain":[{"path":"bins/eliotd/src/main.rs","line":61,"symbol":"run","text":"fn run() -> Result<(), String> {"},{"path":"bins/eliotd/src/main.rs","line":158,"symbol":"run_loop","text":"async fn run_loop("},{"path":"bins/eliotd/src/main.rs","line":163,"symbol":"run_loop","text":"        tokio::select! {"},{"path":"bins/eliotd/src/main.rs","line":170,"symbol":"claim_agent_activation_ticket","text":"                    .claim_agent_activation_ticket()"},{"path":"bins/eliotd/src/main.rs","line":181,"symbol":"submit_agent_activation_decision","text":"                        && let Err(error) = kernel.submit_agent_activation_decision(&decision).await"},{"path":"bins/eliotd/src/main.rs","line":190,"symbol":"run_loop","text":"            () = tokio::time::sleep(std::time::Duration::from_secs(5)) => {"}],"gate_condition":"valid protected descriptor, Kernel connection, and an available activation ticket or health interval","effect_boundary":"daemon activation-decision submission and Kernel health heartbeat after authenticated startup","falsifier":"a valid descriptor and admitted Kernel connection can no longer reach activation-ticket claim or activation-decision submission"}
]
'@
...
try {
    if ($SelfTest) { Invoke-GeneratorSelfTest; exit 0 }
    ...
    $structuredResult = [ordered]@{
        ...
        verification = [ordered]@{ kind = 'structured_evidence_only'; result = 'PASS'; commands = @('pwsh -NoProfile -File scripts/gen-composition-roots.ps1 -Check','pwsh -NoProfile -File scripts/gen-composition-roots.ps1 -SelfTest','pwsh -NoProfile -File scripts/verify-composition-roots.ps1 -SelfTest','pwsh -NoProfile -File scripts/verify-composition-roots.ps1'); self_tests = @('generator predicate and determinism self-test passed','synthetic tamper rejected','synthetic missing target rejected','synthetic duplicate target rejected','classification/predicate mismatch rejected','omitted commit_canonical caller rejected','cached plus nonignored-untracked Rust census enforced','CRLF fixture distinguished from canonical LF'); proof_ceiling = 'static composition-root reachability; no live runtime PASS claim; no TerminalWorkUpdate claim' }
        integration_disposition = [ordered]@{ accepted_findings = @('Current cargo metadata census is 23 binary targets, not the historical 21.','Every target has a checked manifest/main/call-chain witness, gate condition, effect boundary and falsifier.','The registry-wide canonical commit invariant records one workspace-Rust definition and zero callers.'); deferred = @('No cutover or architecture decision is made by this registry.','Static useful_work is not a live observed PASS.','The W1 result-envelope ContractChallenge remains open; this artifact is evidence only.') }
    }
