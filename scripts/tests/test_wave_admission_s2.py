"""#966 S2 wave admission matrix: 20-case integration proof for six Curation leaves.

Admitting wave T8-AS2 moves exactly these six packages from root ``exclude``
to root ``members`` in one serialized turn (issue #966):

- crates/smart/eliot-dreamer-failure            (leaf #663, order 26)
- crates/smart/eliot-dreamer-structure-repair   (leaf #665, order 27)
- crates/smart/eliot-dreamer-reconsolidation    (leaf #667, order 28)
- crates/smart/eliot-dreamer-accessibility      (leaf #669, order 29)
- crates/smart/eliot-dreamer-memory-repair      (leaf #671, order 30)
- crates/smart/eliot-dreamer-curation           (leaf #684, order 31)

Frozen bounded fixtures:
- scripts/testdata/work-unit-gate/wave-s2/candidate.json
- scripts/testdata/work-unit-gate/wave-s2/baseline.json
...
Documented runners (repo root, ``CARGO_TARGET_DIR`` set per owner disk rule)::
    ...
    python -m py_compile scripts/tests/test_wave_admission_s2.py
    python -m unittest scripts.tests.test_wave_admission_s2 -v

Deterministic, no network, no stubs. Declared denominator: 20 cases, exactly
1..20, one method per ``# WORK_UNIT_CASE: 966/<case>`` marker.
"""
...
CURATION = "eliot-dreamer-curation"
OWNED_PATHS = frozenset({
    "Cargo.toml",
    "Cargo.lock",
    "crates/smart/eliot-dreamer-failure/Cargo.toml",
    "crates/smart/eliot-dreamer-failure/module.toml",
    "crates/smart/eliot-dreamer-structure-repair/Cargo.toml",
    "crates/smart/eliot-dreamer-structure-repair/module.toml",
    "crates/smart/eliot-dreamer-reconsolidation/Cargo.toml",
    "crates/smart/eliot-dreamer-reconsolidation/module.toml",
    "crates/smart/eliot-dreamer-accessibility/Cargo.toml",
    "crates/smart/eliot-dreamer-accessibility/module.toml",
    "crates/smart/eliot-dreamer-memory-repair/Cargo.toml",
    "crates/smart/eliot-dreamer-memory-repair/module.toml",
...
def cargo(*args: str, timeout: int = 600) -> subprocess.CompletedProcess:
    ...
    if "CARGO_TARGET_DIR" not in env:
        target = ROOT.parent / "work-966-target"
        if target.is_dir():
...
class TestWaveAdmissionS2(unittest.TestCase):
    """20 substantive cases for issue #966."""
    ...
    def test_05_package_ready_proof_precedes_and_ignores_this_issue(self) -> None:
        ...
        for item in self.six:
            ...
            for rs in proofs:
                ...
                self.assertNotIn("T8-AS2", text, str(rs))
                self.assertNotIn("admit-s2", text, str(rs))
                self.assertNotIn("test_wave_admission_s2", text, str(rs))

    # WORK_UNIT_CASE: 966/6
    ...
    def test_07_compile_graph_acyclic_and_contract_only(self) -> None:
        ...
        poison = dict(edges)
        poison["eliot-dreamer-failure"] = set(poison["eliot-dreamer-failure"]) | {"eliot-dreamer-curation"}
        poison["eliot-dreamer-curation"] = set(poison["eliot-dreamer-curation"]) | {"eliot-dreamer-failure"}
        with self.assertRaises(ValueError):
            topo_sort(poison)

    # WORK_UNIT_CASE: 966/8
    def test_08_five_handlers_cannot_import_grounding_or_peer_algorithms(self) -> None:
        peers = {"eliot_dreamer_" + h.split("eliot-dreamer-")[1].replace("-", "_")
                 for h in FIVE_HANDLERS} | {"eliot_dreamer_curation"}
        for item in self.six:
    ...
    # WORK_UNIT_CASE: 966/9
    def test_09_curation_fan_in_cannot_import_concrete_handlers_or_screen(self) -> None:
        item = next(i for i in self.six if i["name"] == CURATION)
        tokens = crate_tokens_in_rs(item["crate_path"])
        banned = (FORBIDDEN_ALGO
                  | {"eliot_memory_curation_screen"}
                  | {"eliot_dreamer_" + h.split("eliot-dreamer-")[1].replace("-", "_")
                     for h in ALL_TEN_HANDLERS})
        self.assertEqual(tokens & banned, set())
        manifest = load_toml(f"{item['crate_path']}/Cargo.toml")
        declared: set[str] = set()
        for table in ("dependencies", "dev-dependencies"):
            declared |= set(manifest.get(table, {}))
        for forbidden_crate in banned:
            dash_name = forbidden_crate.replace("_", "-")
            self.assertNotIn(dash_name, declared, f"{item['name']} must not depend on {dash_name}")

    # WORK_UNIT_CASE: 966/10
    ...
    def test_12_inheritance_preserves_versions_features_lints(self) -> None:
        ...
        for item in self.six:
            ...
            for key, val in (("version", "0.1.0"), ("edition", "2024"),
                             ("rust-version", "1.94"), ("license", "MIT")):
                self.assertIn(package[key], ({"workspace": True}, val), f"{item['name']}.{key}")
            self.assertEqual(manifest["lints"], {"workspace": True})
            self.assertNotIn("workspace", manifest, item["name"])
            self.assertIn(package.get("publish", False), (False, None))
            for table in ("dependencies", "dev-dependencies"):
            ...
            for table in ("dependencies", "dev-dependencies"):
                ...
                for dep, live_spec in live_section.items():
                    ...
                    if dep in ("schemars", "serde_json", "serde"):
    ...
    def test_17_descriptors_decode_and_suites_execute_pass(self) -> None:
        ...
        self.assertFalse(is_workspace_member(self.metadata, "eliot-dreamer-source-assurance"))

    # WORK_UNIT_CASE: 966/18
