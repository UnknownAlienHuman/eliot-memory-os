"""Canonical Python case bindings for work unit #911 (D-INT-RUNTIME isolated provider).

Declared denominator: exactly 29 cases, 911/1..29, each marked with
``# WORK_UNIT_CASE: 911/<n>`` immediately above its unittest method.
...
_contained_plan: pwsh -NoProfile -NonInteractive -File <Runtime suite abs>
...
Runtime. It requires ``scripts/tests/IntegrationHarness.Runtime.Tests.ps1``;
when that file is absent the seam fails closed and these cases honestly fail --
never fake a pass. No live topology is started and nothing is downloaded.
"""
...
FIXTURES = ROOT / "scripts" / "testdata" / "integration" / "runtime-provider"

RUNTIME_SUITE_NAME = "IntegrationHarness.Runtime"
RUNTIME_SUITE_PATH = "scripts/tests/IntegrationHarness.Runtime.Tests.ps1"

MIN_CASE_ID = 1
MAX_CASE_ID = 29

SCHEMA_VERSION = "harness-runtime-case-result-v1"
...
PYTHON_SUITE_MODULE = "scripts.tests.test_integration_harness_runtime"
PYTHON_TEST_ROOTS = ("scripts/tests",)

MAX_SUITE_BYTES = 1048576
_HEX64 = frozenset("0123456789abcdef")

_CONTAINED_DESCRIPTOR_FILENAME = ".github/work-units/911.toml"
_CONTAINED_DESCRIPTOR_TOML = b"""\
schema_version = "eliot-work-unit-descriptor-v2"
identity = {value = "work-unit-911"}
issue = {repository = {owner = "UnknownAlienHuman", name = "eliot-memory-os"}, number = 911}
unit = {value = "D-INT-RUNTIME"}
mode = "python-unittest"
source_roots = [{value = "scripts/integration/IntegrationHarness.Runtime.psm1"}]
test_roots = [{value = "scripts/tests/test_integration_harness_runtime.py"}]
matrix_cases = 29
...
module = {value = "scripts.tests.test_integration_harness_runtime"}
requirements = {source_floor = 1, public_floor = 1, test_floor = 29, required_guards = [{value = "bounded"}]}
bounds = {wall_ms = 180000, idle_ms = 60000, output_bytes = 16384, line_bytes = 4096, discovery_tests = 29, child_processes = 4}
"""


class HarnessError(Exception):
    """Stable Runtime seam failure. Never a pass; carries a code, never secrets."""
...
def resolve_case(*, suite_path: str, case_id: int) -> CaseBinding:
    """Bind the finite registered Runtime (suite, case) pair to content digest."""
    if suite_path != RUNTIME_SUITE_PATH:
    ...
    if type(case_id) is not int or not MIN_CASE_ID <= case_id <= MAX_CASE_ID:
        raise HarnessError("CASE_ID_BOUND",
                           "case id is not a registered integer 1..29")
    ...
    return CaseBinding(suite=RUNTIME_SUITE_NAME, suite_path=suite_path,
...
def _verify_fields(result: dict, *, case_id: int, expected_digest: str,
    ...
    if result["suite"] != RUNTIME_SUITE_NAME:
    ...
    if result["identity"] != f"911/{case_id}":
...
def run_case(case_id: int, *, python_identity: str,
    ...
    """Execute the registered Runtime case and return its verified result."""
    plan = _contained_plan()
    _check_python_suite_binding()
    _check_python_identity(python_identity)
    binding = resolve_case(suite_path=RUNTIME_SUITE_PATH, case_id=case_id)
    env = _child_env()
...
CASE_METHODS = (
    "test_911_01_provider_schema_revision_class",
    "test_911_02_unknown_class_component_target_revision",
    "test_911_03_no_unrequested_launch",
    "test_911_04_mutation_free_plan",
    "test_911_05_artifact_binding",
    "test_911_06_stale_mixed_missing_substituted_artifact",
    "test_911_07_arbitrary_command_unrepresentable",
    "test_911_08_unique_identities",
    "test_911_09_path_reparse_symlink_reserved_foreign",
    "test_911_10_principal_acl",
    "test_911_11_store_git_receipts_unfabricable",
    "test_911_12_registration_containment_before_execution",
    "test_911_13_observed_not_readiness",
    "test_911_14_handshake_substitution",
    "test_911_15_generation_fence_from_owner",
    "test_911_16_stale_generation_fence",
    "test_911_17_full_denominator",
    "test_911_18_crash_timeout_blocks_dependents",
    "test_911_19_unknown_reconciliation",
    "test_911_20_owner_only_reset",
    "test_911_21_bounded_evidence_identity",
    "test_911_22_canary_absence",
    "test_911_23_reverse_order_shutdown",
    "test_911_24_parent_exit_not_cleanup",
    "test_911_25_owned_tree_only_forced_stop",
    "test_911_26_unknown_stop_preserves_failure_idempotent",
    "test_911_27_concurrent_run_isolation",
    "test_911_28_no_foreign_mutation_test_pass_authority",
    "test_911_29_governor_config_provenance_separation",
)
...
class TestIntegrationHarnessRuntime(unittest.TestCase):
    ...
    def _assert_live_result(self, case_id: int, result: CaseResult) -> None:
        ...
        self.assertEqual(result.suite, RUNTIME_SUITE_NAME)
        self.assertEqual(result.schema_version, SCHEMA_VERSION)
        self.assertEqual(result.identity, f"911/{case_id}")
    ...
    def _run_live(self, case_id: int) -> CaseResult:
        ...
        try:
            ...
            self.fail(f"honest Runtime seam failure for 911/{case_id} [{exc.code}]: {exc}")
        self._assert_live_result(case_id, result)
        return result

    # WORK_UNIT_CASE: 911/1
    def test_911_01_provider_schema_revision_class(self):
        self._run_live(1)

    # WORK_UNIT_CASE: 911/2
    def test_911_02_unknown_class_component_target_revision(self):
        self._run_live(2)

    # WORK_UNIT_CASE: 911/3
    def test_911_03_no_unrequested_launch(self):
        self._run_live(3)

    # WORK_UNIT_CASE: 911/4
    def test_911_04_mutation_free_plan(self):
        self._run_live(4)

    # WORK_UNIT_CASE: 911/5
    def test_911_05_artifact_binding(self):
        self._run_live(5)

    # WORK_UNIT_CASE: 911/6
    def test_911_06_stale_mixed_missing_substituted_artifact(self):
        self._run_live(6)

    # WORK_UNIT_CASE: 911/7
    def test_911_07_arbitrary_command_unrepresentable(self):
        self._run_live(7)

    # WORK_UNIT_CASE: 911/8
    def test_911_08_unique_identities(self):
        self._run_live(8)

    # WORK_UNIT_CASE: 911/9
    def test_911_09_path_reparse_symlink_reserved_foreign(self):
        self._run_live(9)

    # WORK_UNIT_CASE: 911/10
    def test_911_10_principal_acl(self):
        self._run_live(10)

    # WORK_UNIT_CASE: 911/11
    def test_911_11_store_git_receipts_unfabricable(self):
        self._run_live(11)

    # WORK_UNIT_CASE: 911/12
    def test_911_12_registration_containment_before_execution(self):
        self._run_live(12)

    # WORK_UNIT_CASE: 911/13
    def test_911_13_observed_not_readiness(self):
        self._run_live(13)

    # WORK_UNIT_CASE: 911/14
    def test_911_14_handshake_substitution(self):
        self._run_live(14)

    # WORK_UNIT_CASE: 911/15
    def test_911_15_generation_fence_from_owner(self):
        self._run_live(15)

    # WORK_UNIT_CASE: 911/16
    def test_911_16_stale_generation_fence(self):
        self._run_live(16)

    # WORK_UNIT_CASE: 911/17
    def test_911_17_full_denominator(self):
        self._run_live(17)

    # WORK_UNIT_CASE: 911/18
    def test_911_18_crash_timeout_blocks_dependents(self):
        self._run_live(18)

    # WORK_UNIT_CASE: 911/19
    def test_911_19_unknown_reconciliation(self):
        self._run_live(19)

    # WORK_UNIT_CASE: 911/20
    def test_911_20_owner_only_reset(self):
        self._run_live(20)

    # WORK_UNIT_CASE: 911/21
    def test_911_21_bounded_evidence_identity(self):
        self._run_live(21)

    # WORK_UNIT_CASE: 911/22
    def test_911_22_canary_absence(self):
        self._run_live(22)

    # WORK_UNIT_CASE: 911/23
    def test_911_23_reverse_order_shutdown(self):
        self._run_live(23)

    # WORK_UNIT_CASE: 911/24
    def test_911_24_parent_exit_not_cleanup(self):
        self._run_live(24)

    # WORK_UNIT_CASE: 911/25
    def test_911_25_owned_tree_only_forced_stop(self):
        self._run_live(25)

    # WORK_UNIT_CASE: 911/26
    def test_911_26_unknown_stop_preserves_failure_idempotent(self):
        self._run_live(26)

    # WORK_UNIT_CASE: 911/27
    def test_911_27_concurrent_run_isolation(self):
        self._run_live(27)

    # WORK_UNIT_CASE: 911/28
    def test_911_28_no_foreign_mutation_test_pass_authority(self):
        self._run_live(28)

    # WORK_UNIT_CASE: 911/29
    def test_911_29_governor_config_provenance_separation(self):
        self._run_live(29)
