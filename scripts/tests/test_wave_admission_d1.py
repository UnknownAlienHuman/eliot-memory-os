"""#968 D1 wave admission matrix: 20-case integration proof for three brief leaves.

Admitting wave T8-A1 moves exactly these three packages from root ``exclude``
to root ``members`` in one serialized turn (issue #968):
...
admitted via #702 and is a verified no-op of this wave, not an omission
(see ``test_10``).
...
- ``six``/``five`` -> ``three`` (three leaves, agent_orders 7..9);
...
- the handler/curation-contract legs become clarification/router and
  ...
  grounding imports; neither brief imports the other; only the
  ImplementationBrief consumes eliot-conformance-contracts);
- ``test_10`` additionally verifies the #702 orientation admission as a
  byte-identical no-op (member once, router ADMITTED, no diff touches it);
- ``test_12`` additionally allows the workspace-inheritance normalization
  the three D1 leaves receive at admission (base manifests predate the
  orientation pattern; live manifests match it; dependency names unchanged);
- ``test_16`` keeps the S2 in-memory form (``check`` plus double
  in-memory render, no file mutation);
- counts 160/13 -> 166/7 -> 169/4, total 142 -> 114, unit T8-AS2 -> T8-A1.
...
THREE = (CLARIFICATION, ARCH_BRIEF, IMPL_BRIEF)
...
def validate_wave_state(members: list[str], exclude: list[str],
                        ...
                        three_paths: list[str]) -> list[str]:
    """Real admission validator shared by positive and negative legs."""
    errors: list[str] = []
    if sorted(set(three_paths)) != sorted(three_paths):
        errors.append("three denominator has duplicates")
    ...
    for item in load_fixture("candidate.json")["three"]:
...
def validate_challenges(open_entries: list[dict], three_names: set[str]) -> list[str]:
    errors: list[str] = []
    for entry in open_entries:
        if three_names & set(entry.get("needed_by", [])):
...
class TestWaveAdmissionD1(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.three = cls.candidate["three"]
        cls.three_paths = [item["crate_path"] for item in cls.three]
        cls.three_names = [item["name"] for item in cls.three]
    ...
    def test_01_exact_three_package_cell_denominator(self) -> None:
        ws = root_workspace()
        self.assertEqual(len(self.three_paths), 3)
        self.assertEqual(len(set(self.three_paths)), 3)
        self.assertEqual(sorted(i["leaf_issue"] for i in self.three),
                         [630, 649, 651])
        self.assertEqual(sorted(i["agent_order"] for i in self.three),
                         [7, 8, 9])
        for item in self.three:
        ...
        live_cells = {load_toml(f"{p}/module.toml")["module_id"] for p in self.three_paths}
        self.assertEqual(len(live_cells), 3)
    ...
    def test_04_unresolved_challenge_or_second_writer_blocks_activation(self) -> None:
        ...
        blocking = [e for e in open_entries
                    ...
                    and set(self.three_names) & set(e.get("needed_by", []))]
        self.assertEqual(blocking, [])
        self.assertEqual(validate_challenges([], set(self.three_names)), [])
        poison = [{"id": "CC-X", "needed_by": [self.three_names[0]]}]
        self.assertTrue(validate_challenges(poison, set(self.three_names)))
    ...
    def test_07_compile_graph_acyclic_and_contract_only(self) -> None:
        ...
        for name in self.three_names:
            self.assertIn(name, order)
        forbidden = FORBIDDEN_ALGO | three_tokens()
        for item in self.three:
            manifest = load_toml(f"{item['crate_path']}/Cargo.toml")
    ...
    def test_08_clarification_routes_without_grounding_or_peer_imports(self) -> None:
        item = next(i for i in self.three if i["name"] == CLARIFICATION)
        ...
        self.assertEqual(tokens & three_tokens() - {own}, set(), item["name"])
        manifest = load_toml(f"{item['crate_path']}/Cargo.toml")
        declared: set[str] = set()
        ...
        self.assertEqual(poisoned & (FORBIDDEN_ALGO | three_tokens()),
                         {"eliot_dreamer_claim_grounding"})
    ...
    def test_10_orientation_noop_verified_and_three_admitted(self) -> None:
        ...
        modules = {p: {"status": load_toml(f"{p}/module.toml")["status"]}
                   for p in self.three_paths}
        self.assertEqual(validate_wave_state(
            ws["members"], ws["exclude"], modules,
            set(lock_packages()), self.three_paths), [])
        for path in self.three_paths:
        ...
        stale_errors = validate_wave_state(
            ...
            {p: {"status": "PROTOTYPE"} for p in self.three_paths},
            set(lock_packages()), self.three_paths)
    ...
    def test_11_one_member_per_package_no_duplication(self) -> None:
        ...
        names = [load_toml(f"{p}/Cargo.toml")["package"]["name"] for p in self.three_paths]
        self.assertEqual(sorted(names), sorted(self.three_names))
        dup = ws["members"] + [self.three_paths[0]]
        self.assertIn("duplicate workspace member",
                      validate_wave_state(dup, ws["exclude"],
                                          {p: {"status": "ADMITTED"} for p in self.three_paths},
                                          set(lock_packages()), self.three_paths))
    ...
    def test_12_inheritance_preserves_versions_features_lints(self) -> None:
        ...
        for item in self.three:
            ...
            for table in ("dependencies", "dev-dependencies"):
                ...
                # Admission normalizes the D1 leaves to the admitted
                # orientation pattern; dependency names are unchanged, only
                # the shared-version spec form moves to workspace inheritance.
    ...
    def test_13_no_rust_test_router_or_floor_change(self) -> None:
        ...
        leaf_test_prefixes = tuple(f"{p}/tests/" for p in self.three_paths)
    ...
    def test_14_root_and_lock_delta_limited_and_explained(self) -> None:
        ...
        self.assertEqual(plus_names, sorted(self.three_names))
        ...
        for entry in live_lock["package"]:
            if entry["name"] in self.three_names:
    ...
    def test_17_descriptors_decode_and_suites_execute_pass(self) -> None:
        ...
        with self.assertRaises(Exception):
            self.decode_descriptor(
                descriptor_toml(self.three[0]) + b'\nfuture_field = "no"\n',
                f".github/work-units/{self.three[0]['leaf_issue']}.toml")
    ...
    def test_20_failed_admission_proves_no_edge_product_release(self) -> None:
        ws = root_workspace()
        partial_members = [m for m in ws["members"] if m != self.three_paths[0]]
        errors = validate_wave_state(
            partial_members, ws["exclude"],
            {p: {"status": "ADMITTED"} for p in self.three_paths},
            set(lock_packages()), self.three_paths)
        self.assertTrue(any(self.three_paths[0] in e for e in errors))
        stuck = ws["exclude"] + [self.three_paths[1]]
        errors = validate_wave_state(
            ws["members"], stuck,
            {p: {"status": "ADMITTED"} for p in self.three_paths},
            set(lock_packages()), self.three_paths)
        self.assertTrue(any(self.three_paths[1] in e for e in errors))
        self.assertEqual(validate_wave_state(
