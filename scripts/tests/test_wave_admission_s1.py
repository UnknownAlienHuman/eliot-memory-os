"""#965 S1 wave admission matrix: 20-case integration proof for six Curation leaves.

Admitting wave T8-AS1 moves exactly these six packages from root ``exclude``
to root ``members`` in one serialized turn (issue #965):

- crates/smart/eliot-memory-curation-screen  (leaf #588, A-20)
- crates/smart/eliot-dreamer-classification  (leaf #653)
- crates/smart/eliot-dreamer-relation        (leaf #655)
- crates/smart/eliot-dreamer-episode         (leaf #657)
- crates/smart/eliot-dreamer-concept         (leaf #659)
- crates/smart/eliot-dreamer-procedure       (leaf #661)

Frozen bounded fixtures (exact filenames frozen before coding; no others):

- scripts/testdata/work-unit-gate/wave-s1/candidate.json
- scripts/testdata/work-unit-gate/wave-s1/baseline.json
...
Documented runners (repo root, ``CARGO_TARGET_DIR`` set per owner disk rule)::
    ...
    python -m py_compile scripts/tests/test_wave_admission_s1.py
    $env:CARGO_TARGET_DIR='C:/Development/Rust/projects/eliot-swarm/MGR02-target'
    python -m unittest scripts.tests.test_wave_admission_s1 -v

Deterministic, no network, no stubs. Declared denominator: 20 cases, exactly
1..20, one method per ``# WORK_UNIT_CASE: 965/<case>`` marker.
"""
...
FIX = ROOT / "scripts" / "testdata" / "work-unit-gate" / "wave-s1"
BASE_SHA = "9fa4f95057063bfb1fa9d5582227c13d8267c6fa"
ADMISSION_NOTE = "admitted via #965 (T8-AS1) root workspace membership"
FIVE_HANDLERS = (
    "eliot-dreamer-classification",
    "eliot-dreamer-relation",
    "eliot-dreamer-episode",
    "eliot-dreamer-concept",
    "eliot-dreamer-procedure",
)
SCREEN = "eliot-memory-curation-screen"
OWNED_PATHS = frozenset({
    "Cargo.toml",
    "Cargo.lock",
    "crates/smart/eliot-memory-curation-screen/Cargo.toml",
    "crates/smart/eliot-memory-curation-screen/module.toml",
    "crates/smart/eliot-dreamer-classification/Cargo.toml",
    "crates/smart/eliot-dreamer-classification/module.toml",
    "crates/smart/eliot-dreamer-relation/Cargo.toml",
    "crates/smart/eliot-dreamer-relation/module.toml",
    "crates/smart/eliot-dreamer-episode/Cargo.toml",
    "crates/smart/eliot-dreamer-episode/module.toml",
    "crates/smart/eliot-dreamer-concept/Cargo.toml",
    "crates/smart/eliot-dreamer-concept/module.toml",
    "crates/smart/eliot-dreamer-procedure/Cargo.toml",
    "crates/smart/eliot-dreamer-procedure/module.toml",
    "docs/code-navigation/PACKAGE_DOCS_INDEX.md",
    "docs/code-navigation/PROTOTYPE_DOCS_INDEX.md",
    "scripts/tests/test_wave_admission_s1.py",
    "scripts/testdata/work-unit-gate/wave-s1/candidate.json",
    "scripts/testdata/work-unit-gate/wave-s1/baseline.json",
})
...
def validate_wave_state(members: list[str], exclude: list[str],
    ...
    for item in load_fixture("candidate.json")["six"]:
...
class TestWaveAdmissionS1(unittest.TestCase):
    """20 substantive cases for issue #965."""
    ...
    # WORK_UNIT_CASE: 965/1
    def test_01_exact_six_package_cell_denominator(self) -> None:
        ws = root_workspace()
        self.assertEqual(len(self.six_paths), 6)
        self.assertEqual(len(set(self.six_paths)), 6)
        self.assertEqual(sorted(i["leaf_issue"] for i in self.six),
                         [588, 653, 655, 657, 659, 661])
        ...
        live_cells = {load_toml(f"{p}/module.toml")["module_id"] for p in self.six_paths}
        self.assertEqual(len(live_cells), 6)

    # WORK_UNIT_CASE: 965/2
    def test_02_missing_unmerged_unaccepted_leaf_blocks_activation(self) -> None:
        live = []
        for item in self.six:
            merged = git("merge-base", "--is-ancestor",
                         self.candidate["leaf_touch_commit"], BASE_SHA)
    ...
    def test_05_package_ready_proof_precedes_and_ignores_this_issue(self) -> None:
        ...
        self.assertNotIn(self.candidate["leaf_touch_commit"][:12], branch_commits)
        for item in self.six:
            module = load_toml(f"{item['crate_path']}/module.toml")
            ...
            for rs in proofs:
                # Leaf proof carries no wave-integration marker: no T8-AS1 /
                # admit-s1 / suite reference (a plain "#965 is separate" note
                # in procedure docs states the same independence explicitly).
                text = rs.read_text(encoding="utf-8")
                self.assertNotIn("T8-AS1", text, str(rs))
                self.assertNotIn("admit-s1", text, str(rs))
                self.assertNotIn("test_wave_admission_s1", text, str(rs))

    # WORK_UNIT_CASE: 965/6
    ...
    def test_07_compile_graph_acyclic_and_contract_only(self) -> None:
        ...
        forbidden = (FORBIDDEN_ALGO
                     | {"eliot_dreamer_" + h.split("eliot-dreamer-")[1].replace("-", "_")
                        for h in FIVE_HANDLERS}
                     | {"eliot_memory_curation_screen"})
        for item in self.six:
            manifest = load_toml(f"{item['crate_path']}/Cargo.toml")
        ...
        poison = dict(edges)
        poison["eliot-dreamer-relation"] = set(poison["eliot-dreamer-relation"]) | {"eliot-dreamer-concept"}
        poison["eliot-dreamer-concept"] = set(poison["eliot-dreamer-concept"]) | {"eliot-dreamer-relation"}
        with self.assertRaises(ValueError):
            topo_sort(poison)

    # WORK_UNIT_CASE: 965/8
    def test_08_screen_uses_curation_contracts_without_handler_imports(self) -> None:
        item = next(i for i in self.six if i["name"] == SCREEN)
        tokens = crate_tokens_in_rs(item["crate_path"])
        self.assertIn("eliot_memory_curation_contracts", tokens)
        banned = (FORBIDDEN_ALGO | {"eliot_dreamer_contracts"}
                  | {"eliot_dreamer_" + h.split("eliot-dreamer-")[1].replace("-", "_")
                     for h in FIVE_HANDLERS})
        ...
        self.assertIn("eliot-memory-curation-contracts", declared)
        self.assertNotIn("eliot-dreamer-contracts", declared)
        poisoned = set(tokens) | {"eliot_dreamer_contracts"}
        self.assertEqual(poisoned & banned, {"eliot_dreamer_contracts"})

    # WORK_UNIT_CASE: 965/9
    def test_09_handlers_use_validated_contracts_without_peer_or_grounding_imports(self) -> None:
        peers = {"eliot_dreamer_" + h.split("eliot-dreamer-")[1].replace("-", "_")
                 for h in FIVE_HANDLERS} | {"eliot_memory_curation_screen"}
        for item in self.six:
            if item["name"] not in FIVE_HANDLERS:
                continue
            tokens = crate_tokens_in_rs(item["crate_path"])
        ...
        poisoned = crate_tokens_in_rs(
            next(i for i in self.six if i["name"] == FIVE_HANDLERS[0])["crate_path"],
        ) | {"eliot_dreamer_claim_grounding"}
        self.assertIn("eliot_dreamer_claim_grounding", poisoned & FORBIDDEN_ALGO)

    # WORK_UNIT_CASE: 965/10
        ...
        for item in self.six:
            ...
            for table in ("dependencies", "dev-dependencies"):
                ...
                for dep, live_spec in live_section.items():
                    ...
                    if dep in ("schemars", "serde_json"):
                        self.assertEqual(live_spec, base_spec, f"{item['name']}.{dep}")
                    elif isinstance(live_spec, dict) and live_spec.get("workspace") is True:
                        self.assertIn(dep, root_deps, f"{item['name']}.{dep}")
                    else:
    ...
    def test_14_root_and_lock_delta_limited_and_explained(self) -> None:
        ...
        for entry in live_lock["package"]:
            if entry["name"] in self.six_names:
    ...
    def test_16_generated_rows_cover_wave_and_regen_is_byte_identical(self) -> None:
        ...
        self.assertIn("**138**", package_index)
        self.assertIn("**33**", prototype_index)
    ...
    def test_17_descriptors_decode_and_suites_execute_pass(self) -> None:
        ...
        with self.assertRaises(Exception):
            self.decode_descriptor(
                descriptor_toml(self.six[0]) + b'\nfuture_field = "no"\n',
                f".github/work-units/{self.six[0]['leaf_issue']}.toml")
        self.assertFalse(is_workspace_member(self.metadata, "eliot-dreamer-bundle"))

    # WORK_UNIT_CASE: 965/18
