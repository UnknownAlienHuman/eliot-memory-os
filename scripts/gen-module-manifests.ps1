...
$GeneratorVersion = 'gen-module-manifests.ps1/2.0.0'
...
try {
    ...
    $outDir=Split-Path $outputFull -Parent;[IO.Directory]::CreateDirectory($outDir)|Out-Null
    if($Check){if(-not(Test-Path -LiteralPath $outputFull -PathType Leaf)){Fail "inventory missing: $outputFull"};$existing=[IO.File]::ReadAllText($outputFull);if($existing -cne ($json+"`n")){Fail 'generated inventory differs from canonical output'};Write-Output "checked $outputFull"}else{[IO.File]::WriteAllText($outputFull,$json+"`n",$Utf8Strict);Write-Output "generated $(Get-PathRel $root $outputFull) ($($packages.Count) packages)"}
    exit 0
