...
function Assert-ResultEnvelope([string] $Root, $Registry, [string] $RegistryText, [string] $TargetSetDigest) {
    ...
    $commands = @('pwsh -NoProfile -File scripts/gen-composition-roots.ps1 -Check','pwsh -NoProfile -File scripts/verify-composition-roots.ps1 -SelfTest','pwsh -NoProfile -File scripts/verify-composition-roots.ps1')
    if (($result.verification.commands -join "`n") -cne ($commands -join "`n")) { Fail 'W1-07 result command envelope mismatch' }
    $selfTests = @('synthetic tamper rejected','synthetic missing target rejected','synthetic duplicate target rejected','classification/predicate mismatch rejected','omitted commit_canonical caller rejected','cached plus nonignored-untracked Rust census enforced','CRLF fixture distinguished from canonical LF')
    ...
    $accepted = @('Current cargo metadata census is 22 binary targets, not the historical 21.','Every target has a checked manifest/main/call-chain witness, gate condition, effect boundary and falsifier.','The registry-wide canonical commit invariant records one workspace-Rust definition and zero callers.')
